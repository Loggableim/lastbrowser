...
BROWSER_TOOL_SCHEMAS = [
    ...
    {
        "name": "browser_action_v1",
        "description": "Run a versioned browser action sequence against the visible Hermes WebUI browser. Each step can be navigate/open, click, type, scroll, wait, press, back, forward, reload, or stop. Use this when you need several actions in one round-trip and want the user to see the same live browser updating step by step.",
        "parameters": {
            "type": "object",
            "properties": {
                "version": {
                    "type": "integer",
                    "default": 1,
                    "description": "Protocol version. Keep this at 1."
                },
                "steps": {
                    "type": "array",
                    ...
                    "items": {
                        ...
                        "properties": {
                            "action": {
                                "type": "string",
                                "description": "One of navigate, open, click, type, scroll, wait, press, back, forward, reload, or stop."
                            },
                            "ref": {"type": "string"},
...
def browser_navigate(url: str, task_id: Optional[str] = None) -> str:
    ...
    webui_result = _webui_browser_post("navigate", {"url": url}, require_control=True)
...
def browser_snapshot(
    ...
    """
    webui_result = _webui_browser_post("snapshot", {"full": full}, require_control=False)
    if webui_result is not None:
        return webui_result
...
def browser_click(ref: str, task_id: Optional[str] = None) -> str:
    ...
    webui_result = _webui_browser_post("click", {"ref": ref}, require_control=True)
    if webui_result is not None:
        return webui_result
...
def browser_type(ref: str, text: str, task_id: Optional[str] = None) -> str:
    ...
    webui_result = _webui_browser_post("type", {"ref": ref, "text": text}, require_control=True)
    if webui_result is not None:
        return webui_result
...
def browser_scroll(direction: str, task_id: Optional[str] = None) -> str:
    ...
    webui_result = _webui_browser_post("scroll", {"direction": direction}, require_control=True)
    if webui_result is not None:
        return webui_result
...
def browser_back(task_id: Optional[str] = None) -> str:
    ...
    webui_result = _webui_browser_post("back", require_control=True)
    if webui_result is not None:
        return webui_result
...
def browser_press(key: str, task_id: Optional[str] = None) -> str:
    ...
    webui_result = _webui_browser_post("press", {"key": key}, require_control=True)
    if webui_result is not None:
        return webui_result
...
def browser_action_v1(steps: List[Dict[str, Any]], task_id: Optional[str] = None) -> str:
    """Run a versioned browser action sequence in the visible Hermes WebUI browser."""
    if _is_camofox_mode():
        return json.dumps({
            "success": False,
            "error": "browser_action_v1 is not supported in Camofox mode",
        }, ensure_ascii=False)

    payload = {"version": 1, "steps": steps}
    webui_result = _webui_browser_action_post(payload, require_control=True)
    if webui_result is not None:
        return webui_result

    return json.dumps({
        "success": False,
        "error": "WebUI browser action sequence requires the Hermes WebUI browser bridge",
    }, ensure_ascii=False)
...
registry.register(
    name="browser_action_v1",
    toolset="browser",
    schema=_BROWSER_SCHEMA_MAP["browser_action_v1"],
    handler=lambda args, **kw: browser_action_v1(steps=args.get("steps", []), task_id=kw.get("task_id")),
    check_fn=check_browser_requirements,
    emoji="🧭",
)
