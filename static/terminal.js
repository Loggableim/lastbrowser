...
const TerminalStream = {
  ...
  connect(sessionId) {
    ...
    source.addEventListener('exit', ev => {
      ...
      // ── Error detection: store last terminal error for "Fix this?" button ──
      if (code !== 0) {
        window._lastTerminalError = {
          code: code,
          text: exitText || _terminalBufferText(),
          timestamp: Date.now(),
        };
...
// ── Terminal error detection utilities ─────────────────────────────────
// These are used by the "Fix this?" button in tool cards to detect
// terminal command failures from both live streams and settled tool data.

/**
 * Check if a tool call represents a terminal error.
 * @param {Object} tc - A tool call object from S.toolCalls or live SSE
 * @returns {boolean}
 */
function _isTerminalError(tc) {
  if (!tc) return false;
  if (tc.name !== 'terminal' && tc.name !== 'execute_code') return false;
  if (tc.is_error) return true;
  // Fallback: check the preview/snippet for error patterns
  const text = String(tc.preview || tc.snippet || '');
  return /[Ee]rror:|[Tt]raceback|exit code [^0]|FAILED|failed|Command.*not found/i.test(text);
}

/**
 * Extract error context from a failed terminal tool call for the /fix command.
 * Returns a concise but useful error description.
 * @param {Object} tc - A tool call object with name, args, preview, snippet
 * @returns {string} Error context suitable for appending to "/fix "
 */
function _terminalErrorContext(tc) {
  if (!tc) return '';
  const cmd = tc.args && (tc.args.command || tc.args.cmd || '');
  const output = tc.preview || tc.snippet || '';
  // Build a compact error report: the command + the last ~500 chars of output
  let ctx = '';
  if (cmd) ctx += `Command: ${cmd}\n`;
  ctx += 'Error output:\n' + String(output).slice(-500);
  return ctx;
}
