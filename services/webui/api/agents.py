...
def mark_splash_completed(activated_slugs: list[str]) -> dict:
    ...
    try:
        from api.agent_workspace import init_workspaces_for_agents
...
def _load_llm_config():
    ...
    try:
        from api.config import resolve_active_provider_context
...
def chat_with_agent(
    ...
    # Fast-fail when the shared provider context is absent instead of
    # pretending the research/chat succeeded with a fallback assistant reply.
    config = _load_llm_config()
