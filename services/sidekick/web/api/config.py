...
_session_dir_local = threading.local()

def get_session_dir() -> Path:
    """Return the session directory for the current thread.

    Falls back to the module-level ``SESSION_DIR`` when no per-thread
    override has been set (e.g. outside an HTTP request context).
    """
    ws_path = getattr(_session_dir_local, 'path', None)
    return Path(ws_path) if ws_path else SESSION_DIR


def set_session_dir(path: str | Path | None) -> None:
    """Override ``get_session_dir()`` for the current thread.

    Accepts a directory path or ``None`` to clear the override and
    fall back to the module-level ``SESSION_DIR``.
    """
    _session_dir_local.path = str(path) if path is not None else None


def clear_session_dir() -> None:
    """Convenience: remove the per-thread override."""
    try:
        del _session_dir_local.path
...
def reload_config() -> None:
    ...
    with _cfg_lock:
        # Save backup before clearing to survive failed loads (problem #23)
        _backup = copy.deepcopy(_cfg_cache) if _cfg_cache else {}
        _cfg_cache.clear()
        config_path = _get_config_path()
        ...
        # If the new load produced an empty cache, restore from backup (problem #23)
        if not _cfg_cache and _backup:
            _cfg_cache.update(_backup)
            logger.warning("reload_config: restored from backup after failed load")
...
def _resolve_provider_alias(name: str) -> str:
    ...
    try:
        from cli.models import _PROVIDER_ALIASES as _agent_aliases
...
_LOCAL_SERVER_PROVIDERS = {
    ...
    "local-gpu",    # local GPU agent pool provider
    "local-cpu",    # local CPU agent pool provider
    "local-qwen",   # local Qwen agent pool provider
...
def resolve_model_provider(model_id: str) -> tuple:
    ...
    if isinstance(model_cfg, dict):
        ...
        config_provider = _resolve_active_provider_from_config_or_auth(model_cfg)
    ...
    # Portable/local configs may expose selectable provider aliases under
    # ``providers`` and store the real model/base_url on that provider entry:
    #
    #   model: local-gpu
    #   providers:
    #     local-gpu:
    ...
    #       model: Dolphin3...
    #
    # Treat selecting ``local-gpu`` as selecting that provider entry, not as a
    # literal model name to be sent to the active cloud provider.
    providers_cfg = cfg.get("providers", {})
    if isinstance(providers_cfg, dict):
        provider_entry = providers_cfg.get(model_id)
        if isinstance(provider_entry, dict):
            provider_model = str(provider_entry.get("model") or "").strip() or model_id
            provider_base_url = str(provider_entry.get("base_url") or "").strip().rstrip("/") or None
            if provider_base_url:
                return provider_model, f"custom:{model_id}", provider_base_url
...
def _read_auth_store() -> dict:
    ...
    try:
        ...
        loaded = _j.loads(auth_store_path.read_text(encoding="utf-8"))
        return loaded if isinstance(loaded, dict) else {}
    except Exception:
...
def _runtime_provider_status(requested: str | None = None) -> dict:
    ...
    try:
        ...
        logger.debug("Failed to resolve runtime provider status", exc_info=True)
        return {}


def _provider_setup_error_payload() -> dict:
    return {
...
def resolve_active_provider_context(
    config_data: dict | None = None,
    *,
    include_runtime: bool = True,
) -> dict:
    """Resolve provider context in one shared order.

    Order:
      1. config.yaml model.provider
      2. auth.json active_provider
      3. runtime provider status
    """
    ...
    if isinstance(model_cfg, str):
        ...
        if provider:
            provider = _resolve_configured_provider_id(
                provider,
                active_cfg,
                base_url=base_url,
            )
    ...
    if not provider:
        ...
        auth_provider = auth_store.get("active_provider") if isinstance(auth_store, dict) else None
        if auth_provider:
            provider = _resolve_configured_provider_id(
                auth_provider,
                active_cfg,
    ...
    if include_runtime and not provider:
        runtime = _runtime_provider_status()
...
def _load_auth_store() -> dict:
    """Return the active profile auth store without exposing credential values."""
    auth_store_path = _get_auth_store_path()
    if not auth_store_path.exists():
        return {}
    try:
        ...
        data = _j.loads(auth_store_path.read_text(encoding="utf-8"))
        return data if isinstance(data, dict) else {}
    except Exception:
        logger.debug("Failed to load auth store from %s", auth_store_path, exc_info=True)
        return {}


def _resolve_active_provider_from_config_or_auth(model_cfg: dict | None = None) -> str | None:
    """Resolve the active provider consistently across model and runtime paths."""
    model_cfg = model_cfg if isinstance(model_cfg, dict) else (cfg.get("model", {}) or {})
    ...
    return _resolve_configured_provider_id(
...
def _cleanup_stale_models_cache_tmp_files() -> None:
    """Remove leftover temp files from interrupted /api/models cache writes."""
    try:
        parent = _models_cache_path.parent
        pattern = _models_cache_path.name + ".*.tmp"
        for tmp_path in parent.glob(pattern):
            try:
                if tmp_path.is_file():
                    tmp_path.unlink()
            except OSError:
                logger.debug("failed to remove stale models cache tmp file %s", tmp_path, exc_info=True)
    except Exception:
        logger.debug("failed to scan stale models cache tmp files", exc_info=True)


_cleanup_stale_models_cache_tmp_files()
...
def _save_models_cache_to_disk(cache: dict) -> None:
    ...
    try:
        ...
        tmp_path = _models_cache_path.with_name(_models_cache_path.name + f".{os.getpid()}.tmp")
        with open(tmp_path, "w", encoding="utf-8") as f:
            json.dump(payload, f, indent=2)
        os.replace(str(tmp_path), str(_models_cache_path))
    except Exception:
        try:
...
def get_available_models() -> dict:
    ...
    def _build_available_models_uncached() -> dict:
        ...
        provider_context = resolve_active_provider_context(cfg)
        active_provider = provider_context.get("provider")
        if provider_context.get("model") and not default_model:
            default_model = str(provider_context.get("model") or "")
        if provider_context.get("source") == "runtime" and provider_context.get("model"):
            default_model = str(provider_context.get("model") or default_model)

        # 2. Read auth store (credential_pool inspection)
        auth_store = _read_auth_store()
        ...
        try:
            ...
            if isinstance(_pool, dict) and _pool:
                try:
                    from runtime.credential_pool import load_pool as _load_pool
    ...
    def put_nowait(self, item: tuple[str, object]) -> None:
        with self._lock:
            ...
            if not subscribers:
                ...
                if len(self._offline_buffer) > self._max_backlog:
                    del self._offline_buffer[:-self._max_backlog]
                return
        ...
        for q in subscribers:
            try:
                ...
                # Subscriber too slow — drop oldest items to make room (problem #9)
                try:
                    q.get_nowait()
                    q.put_nowait(item)
                except queue.Empty:
...
def _cleanup_stale_streams() -> int:
    """Remove STREAMS entries whose associated thread is no longer alive.
    ...
    with STREAMS_LOCK:
        ...
        for sid, stream in list(STREAMS.items()):
            # If the stream has a _thread attribute and it's dead, remove it
            thread = getattr(stream, '_thread', None)
            if thread is not None and not thread.is_alive():
                stale.append(sid)
            elif isinstance(stream, dict) and stream.get('_thread'):
                import threading as _thr
                t = stream.get('_thread')
                if isinstance(t, _thr.Thread) and not t.is_alive():
                    stale.append(sid)
        for sid in stale:
...
def _cleanup_stale_cancel_flags(max_age: float = 3600.0) -> int:
    """Remove CANCEL_FLAGS entries older than *max_age* seconds.

    Prevents unbounded CANCEL_FLAGS growth (problem #16).
    """
    import time as _t
    now = _t.time()
    stale = [k for k, v in list(CANCEL_FLAGS.items())
             if isinstance(v, (int, float)) and (now - float(v)) > max_age]
    for k in stale:
...
def save_settings(settings: dict) -> dict:
    ...
    if raw_pw and isinstance(raw_pw, str) and raw_pw.strip():
        ...
        from web.api.auth import _hash_password
    ...
    # Atomic write: tmp + os.replace prevents corruption on crash (problem #22)
    _tmp = SETTINGS_FILE.with_suffix(f'.tmp.{os.getpid()}.{threading.current_thread().ident}')
    _tmp.write_text(
        json.dumps(persisted, ensure_ascii=False, indent=2),
        encoding="utf-8",
    )
    os.replace(str(_tmp), str(SETTINGS_FILE))
...
# Dev mode for experimental features
DEV_MODE = False
