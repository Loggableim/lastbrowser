...
(function () {
  ...
  function avatarColor(id) {
    ...
    /* Returns HTML for a Discord avatar (avatar circle + img/initials).
       Handles animated avatars (a_ prefix -> .gif), default avatars (null -> CDN default),
       and standard avatars (.webp). Falls back to colored initials. */
    if (!author || !author.id) return '';
    if (!size) size = 32;
    var id = author.id;
    var hash = author.avatar;
    var username = author.username || '?';
    var bg = avatarColor(id);
    ...
    if (hash) {
      ...
      return '<img src="' + esc(url) + '" alt="" style="width:100%;height:100%;object-fit:cover;border-radius:50%;" onerror="this.style.display=\'none\';this.parentNode.innerHTML=\'' + initial + '\';">';
    }

    // Default avatar via Discord CDN
    var defIndex = 0;
    ...
    return '<img src="' + defUrl + '" alt="" style="width:100%;height:100%;object-fit:cover;border-radius:50%;" onerror="this.style.display=\'none\';this.parentNode.innerHTML=\'' + initial + '\';">';
  }
  ...
  async function renderOvDashboard(el) {
    ...
    if (stats.error) {
      el.innerHTML = '<div style="padding:12px;color:#ed4245;text-align:center;font-size:12px;">❌ Bot offline</div>';
      return;
    }
  ...
  async function loadChannelTree() {
    ...
    if (!el) return;
    if (data.error) {
      el.innerHTML = '<div class="discord-empty-state">❌ Channels konnten nicht geladen werden</div>';
      return;
    }
    _channelTree = data;
    let html = '';
