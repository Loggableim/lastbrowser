...
from runtime.credential_pool import CredentialPool, PooledCredential, get_custom_provider_pool_key, load_pool
...
def _copilot_runtime_api_mode(model_cfg: Dict[str, Any], api_key: str) -> str:
    ...
    try:
        from cli.models import copilot_model_api_mode
    ...
    *,
    ...
    if provider == "openai-codex":
        ...
        if effective_model and api_mode != "anthropic_messages":
            try:
                from cli.models import azure_foundry_model_api_mode
...
def _get_named_custom_provider(requested_provider: str) -> Optional[Dict[str, Any]]:
    ...
    if not requested_norm.startswith("custom:"):
        try:
            ...
            # the built-in. See tests/sidekick_cli/test_runtime_provider_resolution.py
    ...
    if isinstance(custom_providers, dict):
        logger.warning(
            ...
            "Run 'sidekick doctor' for details."
        )
        return None
...
def _resolve_azure_foundry_runtime(
    *,
    ...
    if not base_url:
        raise AuthError(
            "Azure Foundry requires a base URL. Set it via 'sidekick model' or "
    ...
    if not api_key:
        raise AuthError(
            ...
            "~/.sidekick/.env or run 'sidekick model' to configure."
        )
...
def _resolve_explicit_runtime(
    *,
    ...
    if provider == "anthropic":
        ...
        api_key = explicit_api_key
        if not api_key:
            from runtime.anthropic_adapter import resolve_anthropic_token
    ...
    if provider == "nous":
        ...
        if not api_key:
            creds = resolve_nous_runtime_credentials(
                min_key_ttl_seconds=max(60, int(os.getenv("SIDEKICK_NOUS_MIN_KEY_TTL_SECONDS", "300"))),
                timeout_seconds=float(os.getenv("SIDEKICK_NOUS_TIMEOUT_SECONDS", "30")),
            )
            api_key = creds.get("api_key", "")
...
def resolve_runtime_provider(
    *,
    ...
    if pool and pool.has_credentials():
        ...
        # non-runtime contexts like `sidekick auth list`).  If the key is
        # expired, clear pool_api_key so we fall through to
        # resolve_nous_runtime_credentials() which handles refresh + mint.
        if provider == "nous" and entry is not None and pool_api_key:
            min_ttl = max(60, int(os.getenv("SIDEKICK_NOUS_MIN_KEY_TTL_SECONDS", "300")))
            nous_state = {
            ...
            if not _agent_key_is_usable(nous_state, min_ttl):
                ...
                pool_api_key = ""
        if entry is not None and pool_api_key and has_usable_secret(pool_api_key):
    ...
    if provider == "openai-codex":
        try:
            ...
            return {
                ...
                "base_url": creds.get("base_url", "").rstrip("/"),
                "api_key": creds.get("api_key", ""),
                "source": creds.get("source", "sidekick-auth-store"),
                "last_refresh": creds.get("last_refresh"),
                "requested_provider": requested_provider,
            }
        except AuthError:
    ...
    if provider == "minimax-oauth":
        ...
        if pconfig and pconfig.auth_type == "oauth_minimax":
            from cli.auth import resolve_minimax_oauth_runtime_credentials
    ...
    if provider == "anthropic":
        ...
        if _is_azure_endpoint:
            ...
            # chain.  Accept both `key_env` (Sidekick canonical — matches the
            # custom_providers field name) and `api_key_env` (documented in the
            # Azure Foundry guide and read by most Sidekick-compatible importers).
    ...
    if provider == "bedrock":
        from runtime.bedrock_adapter import (
            has_aws_credentials,
