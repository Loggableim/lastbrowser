...
def _persist_codex_credentials(
    hermes_home: Path,
    token_data: dict[str, Any],
    *,
    source: str = "manual:device_code",
    label: str = "Codex OAuth",
) -> Path:
    ...
    _accept_sources = {"manual:device_code", "oauth_device", "codex_cli_import"}
...
def _read_codex_cli_tokens() -> dict[str, Any] | None:
    """Read existing ChatGPT/Codex CLI tokens without exposing them to the UI."""
    ...
    try:
        ...
        logger.debug("Failed to parse Codex CLI auth file at %s", auth_path, exc_info=True)
        return None
...
def _describe_codex_start_error(exc: Exception) -> str:
    if isinstance(exc, urllib.error.HTTPError):
        ...
        try:
            ...
            if isinstance(error, dict):
                ...
                detail = " ".join(part for part in (err_type, err_message) if part)
        except Exception:
            detail = ""
        ...
        if exc.code == 530:
            ...
            return (
                f"OpenAI Codex login is temporarily unavailable: {status}{suffix}. "
                "This is the upstream ChatGPT auth route, not Lastbrowser. "
                "If Codex CLI is already signed in, Lastbrowser can import ~/.codex/auth.json; "
                "otherwise try again later."
            )
        suffix = f": {detail}" if detail else ""
        return f"OpenAI Codex login failed at auth.openai.com with {status}{suffix}."
    return f"OpenAI Codex login failed before the device code could be created: {exc}"
...
def start_onboarding_oauth_flow(body: dict[str, Any] | None) -> dict[str, Any]:
    ...
    if cli_tokens:
        _persist_codex_credentials(
            hermes_home,
            cli_tokens,
            source="codex_cli_import",
            label="Codex CLI import",
        )
        flow_id = uuid.uuid4().hex
        flow = {
            "provider": "openai-codex",
    ...
    try:
        ...
        raise RuntimeError(_describe_codex_start_error(exc)) from exc
