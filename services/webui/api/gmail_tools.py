...
def _get_workspace_accounts() -> dict:
    ...
    - ``default`` workspace → global ``ACCOUNTS`` dict
    - Any other workspace → only workspace-specific ``gmail.accounts``
      (global accounts are NOT visible in non-default spaces)
    """
    try:
        from api.workspace_isolation import get_active_workspace_slug, get_workspace
        slug = getattr(_REQUEST_WORKSPACE_LOCAL, "slug", None) or get_active_workspace_slug()
        if slug and slug != "default":
            # Non-default workspace: only its own accounts
            ...
            if ws:
                ...
                if isinstance(ws_gmail, dict):
                    ...
                    if isinstance(ws_accounts, dict):
                        ...
                        return result
            # Newer Spaces live in api.space_engine (HERMES_HOME/spaces).
            # Keep Gmail space-scoped instead of falling back to global creds.
            try:
                from api.space_engine import get_workspace as se_get_workspace
                ...
                logger.debug("Failed to load space_engine gmail accounts", exc_info=True)
            return {}
        # Default workspace (including None slug) → global ACCOUNTS first
        if ACCOUNTS:
            return dict(ACCOUNTS)
        # Fallback: check default workspace's config for saved credentials
        result = {}
                ...
                if isinstance(ws_accounts, dict):
                    for acc_name, acc_cfg in ws_accounts.items():
                        if isinstance(acc_cfg, dict):
                            ...
                            if email_addr and password:
                                result[acc_name] = (email_addr, password)
        # Second fallback: check Space Engine config (separate storage location)
        if not result:
            try:
                from api.space_engine import get_workspace as se_get_workspace
...
def _list_accounts():
    ...
    if not all_accounts:
        try:
            from api.workspace_isolation import get_active_workspace_slug
...
def handle_gmail_post(handler, parsed, body) -> bool:
    ...
    if body_ws:
        try:
            from api.space_engine import get_active_workspace_slug
            if not get_active_workspace_slug():
                # Set workspace context directly from body-provided slug
                from api.workspace_isolation import get_or_create_workspace, set_active_workspace as ws_set_active
                from api.config import set_session_dir
                from api.kanban_bridge import set_workspace_kanban
                from api.space_engine import set_active_workspace as sp_set_active
