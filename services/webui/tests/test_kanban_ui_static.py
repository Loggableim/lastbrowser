...
def test_kanban_dashboard_parity_core_controls_are_native():
    ...
    kanban_start = PANELS.find("// ── Kanban panel (read-only) ──")
    kanban_end = PANELS.find("// ── Memory panel ──", kanban_start)
    assert kanban_start != -1, "Missing kanban panel section in static/panels.js"
    assert kanban_end != -1, "Missing memory panel boundary after kanban section"
    kanban_block = PANELS[kanban_start:kanban_end]
    ...
    assert "prompt(" not in kanban_block
    assert "confirm(" not in kanban_block
