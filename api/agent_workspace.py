...
# ── Hermes Agent Subprocess (Punkt 2) ──────────────────────────────────

def spawn_hermes_agent(agent_slug: str, workdir: str, task: str = "",
                       session_id: str = None) -> dict:
    """Starte einen echten Hermes Agent Subprozess im Workspace.

    Der Agent läuft als `hermes -p <slug> chat -q "<task>"` im Workspace.
    Output wird live per SSE gestreamt. User kann live zusehen.

    Args:
        agent_slug: Agent-Slug = Hermes-Profil-Name
        workdir: Arbeitsverzeichnis
        task: Task-Beschreibung (wird als -q query übergeben)
        session_id: Bestehende Session (optional)
    """
    if not session_id:
        session = create_workspace_session(agent_slug, workdir)
        session_id = session["id"]
    ...
    def _run_hermes():
        try:
            ...
            session["event_queue"].put({
                "type": "thinking",
                "data": f"🚀 Starte Hermes Agent '{agent_slug}'...",
                "session_id": session_id,
            })
            ...
            if not hermes_path:
                for guess in [
                    ...
                    if os.path.exists(guess):
                        ...
                        break
            if not hermes_path:
                hermes_path = "hermes"  # PATH vertrauen

            # 2. Profil existiert? Sonst erstellen via Plugin
            profile_dir = HERMES_HOME / "profiles" / profile
            if not (profile_dir / "config.yaml").exists():
                session["event_queue"].put({
                    "type": "action",
                    "data": f"⚙️ Erstelle Hermes-Profil '{profile}'...",
                    "session_id": session_id,
                })
                subprocess.run(
                    [hermes_path, "profile", "create", profile,
                     "--clone-from", "default", "--no-alias"],
                    capture_output=True, text=True, timeout=30,
                )

            # 3. Query bauen
            query = task.strip() or f"Arbeite im Verzeichnis {workdir} und erkunde es. Was siehst du?"

            # 4. Kommando
            cmd = [hermes_path, "-p", profile, "chat", "-q", query]

            # 5. Environment
            env = os.environ.copy()
            env["HERMES_KANBAN_WORKSPACE"] = workdir
            env["AGENT_SLUG"] = agent_slug
            ...
            # 6. Subprozess starten
            process = subprocess.Popen(
                cmd, stdout=subprocess.PIPE, stderr=subprocess.STDOUT,
                cwd=workdir, text=True, bufsize=1, env=env,
            )
            session["process"] = process
            session["process_pid"] = process.pid

            # 7. Output live streamen
            start = time.time()
            ...
            while True:
                ...
                if not line:
                    if process.poll() is not None:
                        break
                    # No output for 60s? Send keepalive
            ...
            session["event_queue"].put({
                "type": "complete",
                "data": f"✅ Hermes Agent beendet (Exit: {exit_code})",
                "session_id": session_id,
            })
            ...
            session["event_queue"].put({
                "type": "error",
                "data": "❌ hermes CLI nicht gefunden",
                "session_id": session_id,
            })
        except Exception as e:
            session["state"] = STATE_ERROR
    ...
    thread.start()
    return {"session_id": session_id, "status": "hermes_spawned", "agent_slug": agent_slug}
