/* =============================================================
   Hermes WebUI - Codex-style Enhancements
...
(function () {
  ...
  const state = {
    sidebarCollapsed: LS('hermes-webui-sidebar-collapsed', false),
    sidebarWidth: LS('hermes-webui-sidebar-width', 280),
  ...
  function enhanceSidebar() {
    ...
    if (rail) {
      const collapseBtn = EL('button', {
        className: 'rail-btn sidebar-collapse-btn has-tooltip',
        'data-tooltip': 'Collapse sidebar',
        'aria-label': 'Toggle sidebar',
        title: state.sidebarCollapsed ? 'Expand sidebar' : 'Collapse sidebar',
        innerHTML: state.sidebarCollapsed ? ICONS.expand : ICONS.collapse,
  ...
  function toggleSidebar() {
    ...
    LSS('hermes-webui-sidebar-collapsed', state.sidebarCollapsed);
    applySidebarState();
    // Update button icon
    const btn = document.querySelector('.sidebar-collapse-btn');
    if (btn) {
      btn.innerHTML = state.sidebarCollapsed ? ICONS.expand : ICONS.collapse;
  ...
  function applySidebarState() {
    const sidebar = document.querySelector('.layout > .sidebar, .sidebar, aside.sidebar');
    if (!sidebar) return;
    document.documentElement.dataset.sidebarCollapsed = state.sidebarCollapsed ? 'true' : 'false';
    ...
    if (state.sidebarCollapsed) {
      ...
      // Hide labels
      sidebar.querySelectorAll('.sidebar-nav .nav-tab span:not(.nav-tab-icon-inner), .session-item .session-title, .panel-head span').forEach(el => {
      ...
      // Show labels
      sidebar.querySelectorAll('[data-orig-display]').forEach(el => {
  ...
  function startResize(e) {
    e.preventDefault();
    const sidebar = document.querySelector('.layout > .sidebar, .sidebar, aside.sidebar');
    if (!sidebar || state.sidebarCollapsed) return;
  ...
  function enhanceSessionList() {
    ...
    const filterBar = EL('div', {
      ...
      EL('button', {
        ...
        innerHTML: ICONS.star + ' Favorites',
        style: {
        ...
        onClick: () => {
          ...
          filterSessions(); // existing function
          // Re-render favorites filter bar
          enhanceSessionList();
        },
      }),
      EL('button', {
        className: 'filter-btn bulk-btn',
        innerHTML: ICONS.check + (state.bulkMode ? ' Done' : ' Select'),
        style: {
    ...
    if (state.selectedSessions.size > 0 && state.bulkMode) {
      filterBar.appendChild(EL('button', {
        innerHTML: ICONS.trash + ' Delete ' + state.selectedSessions.size,
        style: {
  ...
  async function loadMoreSessions() {
    try {
      const base = document.baseURI || location.href;
      const resp = await fetch(`${base}api/sessions?limit=${state.PAGE_SIZE}&offset=${state.page * state.PAGE_SIZE}`);
      const data = await resp.json();
      const list = document.getElementById('sessionList');
  ...
  async function handleApply(code, path, dialog) {
    ...
    try {
      const base = document.baseURI || location.href;
      await fetch(`${base}api/workspace/write`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ path: path.trim(), content: code }),
      });
      dialog.innerHTML = '<span style="color:var(--success,#4ade80);font-size:12px">✓ Written</span>';
      setTimeout(() => dialog.remove(), 1500);
    } catch {
      dialog.innerHTML = '<span style="color:var(--destructive,#ef4444);font-size:12px">✗ Failed</span>';
  ...
  function addAnimationStyles() {
    ...
    style.textContent = `
      ...
      .sidebar, .layout > .sidebar { transition: width 0.2s ease-out; }

      /* Sidebar collapsed state */
      [data-sidebar-collapsed="true"] .sidebar-nav .nav-tab { justify-content: center; }
      [data-sidebar-collapsed="true"] .sidebar-nav .nav-tab span:not(.nav-tab-icon-inner) { display: none !important; }
  ...
  function run() {
    ...
    const retryTimer = setInterval(trySessions, 500);
    trySessions();
