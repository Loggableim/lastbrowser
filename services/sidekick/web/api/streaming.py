"""
Sidekick -- SSE streaming engine and agent thread runner.
...
def _build_agent_thread_env(profile_runtime_env: dict | None, workspace: str, session_id: str, profile_home: str) -> dict:
    ...
    **kwargs alongside an explicit TERMINAL_CWD raises TypeError before the
    ...
    try:
        ...
        _browser_base_url = os.environ.get("SIDEKICK_WEBUI_BROWSER_BASE_URL") or f"http://127.0.0.1:{_WEBUI_PORT}"
    except Exception:
        _browser_base_url = os.environ.get("SIDEKICK_WEBUI_BROWSER_BASE_URL") or "http://127.0.0.1:8787"
    try:
        ...
        _browser_permission_token = browser_permission_token(session_id)
    except Exception:
...
def _aux_title_configured() -> bool:
    ...
    try:
        from runtime.auxiliary_client import _get_auxiliary_task_config
...
def _maybe_schedule_title_refresh(session, put_event, agent):
    ...
    if not last_u and not last_a:
        return
    # Thread guard: skip spawn if too many active threads (problem #7)
    if threading.active_count() < 50:
        ...
        logger.warning("Thread guard blocked background title refresh: %d active threads", threading.active_count())
...
def _merge_display_messages_after_agent_result(previous_display, previous_context, result_messages, msg_text):
    ...
    If Nova returns a normal append-only history, append that delta to
...
def _last_resort_sync_from_core(session, stream_id, agent_lock):
    ...
    Must never raise.
    """
    from web.api.models import _get_profile_home, _apply_core_sync_or_error_marker
    try:
...
def _run_agent_streaming(
    ...
    *,
    ...
    # profile's mcp_servers because os.environ['SIDEKICK_HOME'] hadn't been
    # rewritten yet.  See https://github.com/Loggableim/sidekick/issues/1968.
    ...
    try:
        ...
        if _ws_slug:
            try:
                ...
                _ws_obj = _goc_ws(_ws_slug)
                _ws_obj.sessions_dir.mkdir(parents=True, exist_ok=True)
        ...
        def _clarify_callback_impl(question, choices, sid, cancel_evt, put_event):
            ...
            try:
                from web.api.clarify import submit_pending as _submit_clarify_pending, clear_pending as _clear_clarify_pending
            except ImportError:
                return (
        ...
        try:
            ...
            def on_tool(*cb_args, **cb_kwargs):
                ...
                if event_type == 'tool.completed':
                    ...
                    if name == 'terminal':
                        ...
                        if _is_error:
                            _exit_code = 1
                        from web.api.command_stream import end_command_stream
            ...
            def on_tool_start(tool_call_id, name, args):
                try:
                    ...
                    if name == 'terminal':
                        ...
                        from web.api.command_stream import start_command_stream
                        ...
                        from web.api.command_stream import write_command_stdout
                ...
                try:
                    ...
                    if name == 'terminal':
                        ...
                        if isinstance(function_result, dict):
                            _exit_code = function_result.get('exit_code', 0)
                        elif isinstance(function_result, int):
                            _exit_code = function_result
                        from web.api.command_stream import end_command_stream
                        end_command_stream(session_id, exit_code=_exit_code)
                        # Clear env output callback
                        _env_output_cb = None
            ...
            if ephemeral:
                ...
                from web.api.config import SESSION_AGENT_CACHE, SESSION_AGENT_CACHE_LOCK
                _sig_blob = _json.dumps([
                    ...
                    # at construction time, sourced from SIDEKICK_HOME. Same-session
                ...
                if agent is not None:
                    ...
                    with SESSION_AGENT_CACHE_LOCK:
                        ...
                        SESSION_AGENT_CACHE.move_to_end(session_id)  # LRU: mark as recently used
                        from web.api.config import SESSION_AGENT_CACHE_MAX
            ...
            try:
                ...
                if _ws_obj_2:
                    _pdir = _ws_obj_2.get_project_dir()
                    if _pdir:
                        _pdir_restriction = (
                            f"\n⚠️ SPACE FILE RESTRICTION: This Space is sandboxed to the project directory:\n"
                            f"  {_pdir}\n"
                            "You MUST restrict ALL file operations to paths under this directory.\n"
                            "Do NOT read, write, or execute files outside this directory for ANY reason.\n"
                            "Set your working directory to this path for ALL terminal commands, "
                            "file reads, file writes, searches, and patches."
                        )
                    # ── Agent SOUL.md Injection (Space Engine v2) ────────────
                    # If the session has an agent_slug, load that agent's SOUL.md
                    # from the space's agents/ directory and inject it into the
                    # system prompt. This replaces the old profile-SOUL.md approach.
                    if hasattr(_ws_obj_2, 'agent_soul_path'):
                        _agent_slug_session = getattr(s, 'agent_slug', None) or 'default'
                        _agent_soul = _ws_obj_2.get_agent_soul(_agent_slug_session)
                        _agent_skills = _ws_obj_2.get_agent_skills(_agent_slug_session)
            except Exception:
                pass
            if _pdir_restriction and not sandbox_disabled:
                workspace_system_msg += _pdir_restriction
            # Inject agent SOUL.md into the workspace system message
            if _agent_soul:
                workspace_system_msg = (
                    f"── Agent Role: {getattr(s, 'agent_slug', 'default')} ──\n"
                    f"{_agent_soul}\n\n"
            ...
            if threading.active_count() < 50:
                ...
                logger.warning("Thread guard blocked checkpoint thread for %s: %d active threads", session_id[:8], threading.active_count())
            ...
            if mode == "plan":
                plan_instruction = (
                    ...
                    "Do NOT execute any commands, write any code, or take any actions.\n"
                    "Output only a plan with steps, reasoning, and expected outcomes.\n"
            ...
            with _agent_lock:
                ...
                if not _assistant_added and not _token_sent:
                    ...
                    if _is_quota:
                        ...
                        if _heal_rt is not None:
                            ...
                            with STREAMS_LOCK:
                                AGENT_INSTANCES[stream_id] = agent
                            from web.api.config import SESSION_AGENT_CACHE as _SAC, SESSION_AGENT_CACHE_LOCK as _SAC_L
                            with _SAC_L:
                ...
                if not getattr(s, 'context_length', 0):
                    try:
                        ...
                        try:
                            from runtime.model_metadata import get_model_context_length as _legacy_cl
        ...
        if _exc_is_quota:
            ...
            if not _self_healed:
                ...
                if _heal_rt is not None:
                    ...
                    from web.api.config import SESSION_AGENT_CACHE as _SAC2, SESSION_AGENT_CACHE_LOCK as _SAC2_L
                    with _SAC2_L:
...
def cancel_stream(stream_id: str) -> bool:
    ...
    with streams_lock:
        ...
        try:
            from web.api.clarify import clear_pending as _clear_clarify_pending
...
def _handle_terminal_stream(handler, parsed):
    ...
    """
    from web.api.helpers import bad
    from urllib.parse import parse_qs
    from web.api.command_stream import get_command_stream, cleanup_session
