...
// ── Prompt History (ArrowUp/Down recall of sent messages) ──
// Stores sent messages so the user can arrow-navigate back through them.
// Modeled after Codex CLI / ChatGPT prompt recall behavior.
const _promptHistory = [];
let _promptHistoryIdx = -1;
let _promptHistorySaved = '';

function _addToPromptHistory(text) {
  if (!text || !text.trim()) return;
  // Deduplicate against the last entry
  const last = _promptHistory[_promptHistory.length - 1];
  if (last === text) return;
  _promptHistory.push(text);
  if (_promptHistory.length > 50) _promptHistory.shift();
  _promptHistoryIdx = _promptHistory.length; // point past the end
  _promptHistorySaved = '';
}
...
function _navigatePromptHistory(dir) {
  ...
  const msg = $('msg');
  if (!msg || _promptHistory.length === 0) return;
  if (dir === -1) {
    // ArrowUp: go older
    if (_promptHistoryIdx <= 0) return; // already at oldest
    if (_promptHistoryIdx === _promptHistory.length) {
      // First time entering history: save the current draft
      _promptHistorySaved = msg.value;
    }
    _promptHistoryIdx--;
    msg.value = _promptHistory[_promptHistoryIdx];
  } else {
    // ArrowDown: go newer
    if (_promptHistoryIdx >= _promptHistory.length) return; // already at newest
    _promptHistoryIdx++;
    if (_promptHistoryIdx >= _promptHistory.length) {
      // Back to current draft
      msg.value = _promptHistorySaved;
      _promptHistoryIdx = _promptHistory.length;
    } else {
      msg.value = _promptHistory[_promptHistoryIdx];
    }
  }
...
$('msg').addEventListener('keydown',e=>{
  ...
  // Prompt history: ArrowUp/ArrowDown when dropdown is not open.
  // Only navigate when cursor is at beginning (ArrowUp) or end (ArrowDown)
  // so multi-line editing still works naturally.
  if (e.key === 'ArrowUp') {
    const msg = $('msg');
    if (msg && msg.selectionStart === 0) {
      e.preventDefault();
  ...
  if (e.key === 'ArrowDown') {
    const msg = $('msg');
    if (msg && msg.selectionStart === msg.value.length) {
      e.preventDefault();
      _navigatePromptHistory(1);
      return;
    }
...
(async()=>{
  ...
  try{
    ...
    window._composerMode=(s.composer_mode||'code');
  ...
  if(window._composerMode&&typeof setComposerMode==='function') setComposerMode(window._composerMode);
