"""
Interactive setup wizard for Sidekick Agent.
...
Config files are stored in ~/.sidekick/ for easy access.
"""
...
from cli.nous_subscription import get_nous_subscription_features
from tools.tool_backend_helpers import managed_nous_tools_enabled
from shared.utils import base_url_hostname
from runtime._compat.shim_constants import get_optional_skills_dir

logger = logging.getLogger(__name__)

PROJECT_ROOT = Path(__file__).parent.parent.resolve()

_DOCS_BASE = "https://sidekick-agent.sh/docs"
...
_DEFAULT_PROVIDER_MODELS = {
    ...
    "opencode-go": ["kimi-k2.6", "kimi-k2.5", "glm-5.1", "glm-5", "deepseek-v4-pro", "deepseek-v4-flash", "mimo-v2.5-pro", "mimo-v2.5", "mimo-v2-pro", "mimo-v2-omni", "minimax-m2.7", "minimax-m2.5", "qwen3.6-plus", "qwen3.5-plus"],
    "huggingface": [
...
# display_sidekick_home imported lazily at call sites
...
from cli.cli_output import (  # noqa: E402
    print_error,
    print_info,
    print_success,
    print_warning,
...
def print_noninteractive_setup_guidance(reason: str | None = None) -> None:
    ...
    print()
    print(color("⚕ Sidekick Setup — Non-interactive mode", Colors.CYAN, Colors.BOLD))
    print()
    if reason:
        print_info(reason)
    print_info("The interactive wizard cannot be used here.")
    print()
    print_info("Configure Sidekick using environment variables or config commands:")
    print_info("  sidekick config set model.provider custom")
    print_info("  sidekick config set model.base_url http://localhost:8080/v1")
    print_info("  sidekick config set model.default your-model-name")
    print()
    print_info("Or set OPENROUTER_API_KEY / OPENAI_API_KEY in your environment.")
    print_info("Run 'sidekick setup' in an interactive terminal to use the full wizard.")
    print()
...
def _curses_prompt_choice(question: str, choices: list, default: int = 0, description: str | None = None) -> int:
    ...
    from cli.curses_ui import curses_radiolist
...
def prompt_checklist(title: str, items: list, pre_selected: list = None) -> list:
    ...
    from cli.curses_ui import curses_checklist
...
def _print_setup_summary(config: dict, sidekick_home):
    ...
    try:
        from runtime.auxiliary_client import get_available_vision_backends
    ...
    if _vision_backends:
        ...
        tool_status.append(("Vision (image analysis)", False, "run 'sidekick setup' to configure"))
    ...
    if subscription_features.image_gen.managed_by_nous:
        ...
        _img_backend = None
        try:
            from runtime.image_gen_registry import list_providers
            from cli.plugins import _ensure_plugins_discovered
    ...
    if subscription_features.tts.managed_by_nous:
        ...
        if neutts_ok:
            ...
            tool_status.append(("Text-to-Speech (NeuTTS — not installed)", False, "run 'sidekick setup tts'"))
        ...
        if kittentts_ok:
            ...
            tool_status.append(("Text-to-Speech (KittenTTS — not installed)", False, "run 'sidekick setup tts'"))
    else:
    ...
    if subscription_features.modal.managed_by_nous:
        ...
        if subscription_features.modal.direct_override:
            ...
            tool_status.append(("Modal Execution", False, "run 'sidekick setup terminal'"))
    ...
    if disabled_tools:
        print_warning(
            "Some tools are disabled. Run 'sidekick setup tools' to configure them,"
        )
        from runtime._compat.shim_constants import display_sidekick_home as _dhh
    ...
    print()
    print(f"   {color('sidekick setup', Colors.GREEN)}          Re-run the full wizard")
    print(f"   {color('sidekick setup model', Colors.GREEN)}    Change model/provider")
    print(f"   {color('sidekick setup terminal', Colors.GREEN)} Change terminal backend")
    print(f"   {color('sidekick setup gateway', Colors.GREEN)}  Configure messaging")
    print(f"   {color('sidekick setup tools', Colors.GREEN)}    Configure tool providers")
    print()
    print(f"   {color('sidekick config', Colors.GREEN)}         View current settings")
    print(
        f"   {color('sidekick config edit', Colors.GREEN)}    Open config in your editor"
    )
    print(f"   {color('sidekick config set <key> <value>', Colors.GREEN)}")
    ...
    print()
    print(f"   {color('sidekick', Colors.GREEN)}              Start chatting")
    print(f"   {color('sidekick gateway', Colors.GREEN)}      Start messaging gateway")
    print(f"   {color('sidekick doctor', Colors.GREEN)}       Check for issues")
    print()
...
# between `sidekick tools` and `sidekick setup tools`).
...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    Delegates to ``cmd_model()`` (the same flow used by ``sidekick model``)
    ...
    provider added to ``sidekick model`` is automatically available here.
    ...
    try:
        ...
        print_info("You can try again later with: sidekick model")
    ...
    if not quick and _supports_same_provider_pool_setup(selected_provider):
        try:
            ...
            from cli.auth_commands import auth_add_command
            ...
            print_info(
                "Sidekick can keep multiple credentials for one provider and rotate between"
            )
            print_info(
    ...
    if _vision_needs_setup:
        ...
        print_info("doesn't currently provide one Sidekick can auto-use for vision,")
        ...
        if _vision_idx == 0:  # OpenRouter
            ...
            print_info("Skipped — add later with 'sidekick setup' or configure AUXILIARY_VISION_* settings")
...
def _setup_tts_provider(config: dict):
    ...
    if selected == "nous-openai":
        ...
        if get_env_value("VOICE_TOOLS_OPENAI_KEY") or get_env_value("OPENAI_API_KEY"):
            print_warning(
                "Direct OpenAI credentials are still configured and may take precedence until removed from ~/.sidekick/.env."
            )
        ...
        if not existing:
            ...
            if api_key:
                ...
                print_warning(
                    ...
                    f"sidekick setup model or {_dhh()}/.env to use xAI TTS. "
...
def setup_tts(config: dict):
    """Standalone TTS setup (for 'sidekick setup tts')."""
    _setup_tts_provider(config)
...
def setup_terminal_backend(config: dict):
    ...
    print_info("Choose where Sidekick runs shell commands and code.")
    ...
    if selected_backend == "local":
        ...
        print_info("Requires the optional SDK: pip install 'sidekick-agent[vercel]'")
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                print_warning("Install failed — run manually: pip install 'sidekick-agent[vercel]'")
                if result.stderr:
                    print_info(f"  Error: {result.stderr.strip().splitlines()[-1]}")
...
def _apply_default_agent_settings(config: dict):
    ...
    print_info("  Run `sidekick setup agent` later to customize.")
...
def _setup_telegram():
    ...
    print()
    print_info("📬 Home Channel: where Sidekick delivers cron job results,")
...
def _setup_slack():
    ...
    if existing:
        ...
        if not prompt_yes_no("Reconfigure Slack?", False):
            ...
            if prompt_yes_no(
                ...
                "list? (recommended after `sidekick update`)",
                True,
            ):
    ...
    print()
    print_info("   Full guide: https://sidekick-agent.sh/docs/user-guide/messaging/slack/")
    print()
...
def _write_slack_manifest_and_instruct():
    ...
    """
    try:
        from cli.slack_cli import _build_full_manifest
        from runtime._compat.shim_constants import get_sidekick_home

        manifest = _build_full_manifest(
            bot_name="Sidekick",
            bot_description="Your Sidekick agent on Slack",
        )
        target = Path(get_sidekick_home()) / "slack-manifest.json"
        ...
        print_info(
            "   Re-run `sidekick slack manifest --write` anytime to refresh after "
            "Sidekick adds new commands."
        )
...
def _setup_matrix():
    ...
    if token or get_env_value("MATRIX_PASSWORD"):
        ...
        print()
        print_info("📬 Home Room: where Sidekick delivers cron job results and notifications.")
...
def _setup_mattermost():
    ...
    print()
    print_info("📬 Home Channel: where Sidekick delivers cron job results and notifications.")
    print_info("   To get a channel ID: click channel name → View Info → copy the ID")
    print_info("   You can also set this later by typing /set-home in a Mattermost channel.")
    home_channel = prompt("Home channel ID (leave empty to set later with /set-home)")
    if home_channel:
        save_env_value("MATTERMOST_HOME_CHANNEL", home_channel)
    print_info("   Open config in your editor:  sidekick config edit")
...
def _setup_bluebubbles():
    ...
    print_info("Connects Sidekick to iMessage via BlueBubbles — a free, open-source")
...
def _setup_qqbot():
    ...
    from cli.gateway import _setup_qqbot as _gateway_setup_qqbot
...
def _setup_webhooks():
    ...
    print()
    print_info("   Full guide: https://sidekick-agent.sh/docs/user-guide/messaging/webhooks/")
    print()
    ...
    print_info("   https://sidekick-agent.sh/docs/user-guide/messaging/webhooks/#configuring-routes")
    print()
    print_info("   Open config in your editor:  sidekick config edit")
    print_info("   Open config in your editor:  sidekick config edit")
...
def setup_gateway(config: dict):
    ...
    from cli.gateway import _all_platforms, _platform_status, _configure_platform

    print_header("Messaging Platforms")
    print_info("Connect to messaging platforms to chat with Sidekick from anywhere.")
    ...
    if not selected:
        print_info("No platforms selected. Run 'sidekick setup gateway' later to configure.")
        return
    ...
    if any_messaging:
        ...
        if missing_home:
            ...
            for plat in missing_home:
                print_info(
                    f"     sidekick config set {plat.upper()}_HOME_CHANNEL <channel_id>"
                )
        ...
        if service_running:
            ...
            if prompt_yes_no(
                ...
                try:
                    ...
                    print_info("  You can try manually: sidekick gateway install")
            else:
                print_info("  You can install later: sidekick gateway install")
                if supports_systemd:
                    print_info("  Or as a boot-time service: sudo sidekick gateway install --system")
                print_info("  Or run in foreground:  sidekick gateway")
        else:
            from runtime._compat.shim_constants import is_container
            if is_container():
                print_info("Start the gateway to bring your bots online:")
                print_info("   sidekick gateway run          # Run as container main process")
                print_info("")
                ...
                print_info("Start the gateway to bring your bots online:")
                print_info("   sidekick gateway              # Run in foreground")
...
def setup_tools(config: dict, first_install: bool = False):
    ...
    Both `sidekick setup tools` and `sidekick tools` use the same flow:
    ...
    """
    from cli.tools_config import tools_command
...
def _model_section_has_credentials(config: dict) -> bool:
    ...
    Sources of truth:
      * ``PROVIDER_REGISTRY`` in ``sidekick_cli.auth`` — lists every supported
    ...
    """
    try:
        from cli.auth import get_active_provider
...
def _get_section_config_summary(config: dict, section_key: str) -> Optional[str]:
    ...
    ``get_env_value`` is the module-level import from cli.config
    ...
    if section_key == "model":
        ...
        from cli.gateway import _all_platforms, _platform_status
...
# Config values may have different semantics between OpenClaw and Sidekick.
# Instruction/context files (.md) can contain incompatible setup procedures.
_HIGH_IMPACT_KIND_KEYWORDS = {
    "gateway": "⚠ Gateway/messaging — this will configure Sidekick to use your OpenClaw messaging channels",
    "telegram": "⚠ Telegram — this will point Sidekick at your OpenClaw Telegram bot",
    "slack": "⚠ Slack — this will point Sidekick at your OpenClaw Slack workspace",
    "discord": "⚠ Discord — this will point Sidekick at your OpenClaw Discord bot",
    "whatsapp": "⚠ WhatsApp — this will point Sidekick at your OpenClaw WhatsApp connection",
    "config": "⚠ Config values — OpenClaw settings may not map 1:1 to Sidekick equivalents",
...
def _print_migration_preview(report: dict):
    ...
    if warnings_shown:
        ...
        print()
        print(color("  Note: OpenClaw config values may have different semantics in Sidekick.", Colors.YELLOW))
        print(color("  For example, OpenClaw's tool_call_execution: \"auto\" ≠ Sidekick's yolo mode.", Colors.YELLOW))
...
def _offer_openclaw_migration(sidekick_home: Path) -> bool:
    ...
    print_info("Sidekick can preview what would be imported before making any changes.")
    print()

    if not prompt_yes_no("Would you like to see what can be imported?", default=True):
        print_info(
            "Skipping migration. You can run it later with: sidekick claw migrate --dry-run"
        )
        return False
    ...
    if not prompt_yes_no("Proceed with migration?", default=False):
        print_info(
            "Migration cancelled. You can run it later with: sidekick claw migrate"
        )
        print_info(
...
def run_setup_wizard(args):
    ...
    Supports full, quick, and section-specific setup:
      sidekick setup           — full or quick (auto-detected)
      sidekick setup model     — just model/provider
      sidekick setup tts       — just text-to-speech
      sidekick setup terminal  — just terminal backend
      sidekick setup gateway   — just messaging platforms
      sidekick setup tools     — just tool configuration
      sidekick setup agent     — just agent settings
    """
    ...
    if section:
        for key, label, func in SETUP_SECTIONS:
            if key == section:
                ...
                print(color(f"│     ⚕ Sidekick Setup — {label:<34s} │", Colors.MAGENTA))
                print(
                    color(
                        "└─────────────────────────────────────────────────────────┘",
                        Colors.MAGENTA,
    ...
    print(
        color(
            "│             ⚕ Sidekick Agent Setup Wizard                │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    print(
        color(
            "│  Let's configure your Sidekick Agent installation.       │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    if is_existing:
        ...
        print_success("You already have Sidekick configured.")
        print_info("Running the full wizard — each prompt shows your current value.")
        print_info("Press Enter to keep it, or type a new value to change it.")
        print_info("")
        print_info("Tip: jump straight to a section with 'sidekick setup model|terminal|")
        ...
        setup_mode = prompt_choice("How would you like to set up Sidekick?", [
...
def _offer_launch_chat():
    ...
    print()
    if not prompt_yes_no("Launch sidekick chat now?", True):
        return
...
def _run_first_time_quick_setup(config: dict, sidekick_home, is_existing: bool):
    ...
    the user can customize later via ``sidekick setup <section>``.
    """
    ...
    gateway_choice = prompt_choice(
        ...
        [
            ...
            "Skip — set up later with 'sidekick setup gateway'",
        ],
        0,
    )
    ...
    print()
    print_info("  Configure all settings:    sidekick setup")
    if gateway_choice != 0:
...
def _run_quick_setup(config: dict, sidekick_home):
    ...
    if not has_anything_missing:
        ...
        print()
        print_info("Run 'sidekick setup' and choose 'Full Setup' to reconfigure,")
    ...
    if missing_messaging:
        print()
        print_header("Messaging Platforms")
        print_info("Connect Sidekick to messaging apps to chat from anywhere.")
        print_info("You can configure these later with 'sidekick setup gateway'.")
