...
_UNSUPPORTED_PROVIDER_NOTE = (
    ...
    "when your Lastbrowser profile selects the corresponding provider."
)
...
def _extract_current_provider(cfg: dict) -> str:
    return str(resolve_active_provider_context(cfg).get("provider") or "").strip().lower()


def _extract_current_model(cfg: dict) -> str:
    context = resolve_active_provider_context(cfg)
    if context.get("model"):
        return str(context.get("model") or "").strip()
...
def _extract_current_base_url(cfg: dict) -> str:
    ...
    if context.get("base_url"):
        return _normalize_base_url(str(context.get("base_url") or ""))
    model_cfg = cfg.get("model", {})
    if isinstance(model_cfg, dict):
...
def _status_from_runtime(cfg: dict, imports_ok: bool) -> dict:
    ...
    if not _HERMES_FOUND or not imports_ok:
        ...
        note = (
            "Sidekick is still preparing the local runtime. Finish bootstrap or repair the "
            "runtime install before provider setup will work."
        )
    elif chat_ready:
        state = "ready"
        ...
        note = f"Sidekick is configured and ready to chat via {provider_name}."
        ...
        if provider == "custom" and not base_url:
            ...
            note = (
                ...
                "Complete provider login in Lastbrowser, then reload Sidekick."
            )
        else:
            note = (
                "Sidekick has a saved provider/model selection but still needs the "
...
def get_onboarding_status() -> dict:
    ...
    if not bot_name or bot_name.lower() == "hermes":
        bot_name = "sidekick"
...
def apply_onboarding_setup(body: dict) -> dict:
    ...
    if Path(config_path).exists() and not body.get("confirm_overwrite"):
        return {
            "error": "config_exists",
            "message": (
                "Lastbrowser is already configured (config.yaml exists). "
