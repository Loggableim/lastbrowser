...
Add, remove, or reorder entries here — both `sidekick setup` and
`sidekick` provider-selection will pick up the change automatically.
"""
...
_SIDEKICK_USER_AGENT = f"sidekick-cli/{_SIDEKICK_VERSION}"
...
def _codex_curated_models() -> list[str]:
    ...
    This keeps the gateway /model picker in sync with the CLI `sidekick model`
    flow without maintaining a separate static list.
    """
    from cli.codex_models import DEFAULT_CODEX_MODELS, _add_forward_compat_models
...
# $SIDEKICK_HOME/models_dev_cache.json as of 2026-04-28. Whenever xAI renames
...
def _xai_curated_models() -> list[str]:
    ...
    Reads $SIDEKICK_HOME/models_dev_cache.json directly (no network) so this
    runs at import time without blocking. Falls back to ``_XAI_STATIC_FALLBACK``
    when the cache is empty or unreadable. Sidekick refreshes the cache from
    ...
    """
    try:
        from runtime.models_dev import _load_disk_cache
...
# Void — Nous Portal free-model and account tier helpers removed during migration.


# ---------------------------------------------------------------------------
# Canonical provider list — single source of truth for provider identity.
# Every code path that lists, displays, or iterates providers derives from
# this list:  sidekick model, /model, list_authenticated_providers.
#
# Fields:
#   slug        — internal provider ID (used in config.yaml, --provider flag)
#   label       — short display name
#   tui_desc    — longer description for the `sidekick model` interactive picker
# ---------------------------------------------------------------------------

class ProviderEntry(NamedTuple):
    slug: str
    label: str
    tui_desc: str   # detailed description for `sidekick model` TUI
...
def get_default_model_for_provider(provider: str) -> str:
    ...
    model a user would be offered first in the ``sidekick model`` picker.

    Used as a fallback when the user has configured a provider but never
    selected a model (e.g. ``sidekick auth add openai-codex`` without
    ``sidekick model``).
    """
...
def _openrouter_model_supports_tools(item: Any) -> bool:
    ...
    sidekick-agent is tool-calling-first — every provider path assumes the model
...
def fetch_openrouter_models(
    ...
    *,
    ...
    try:
        from cli.model_catalog import get_curated_openrouter_models
    ...
    for preferred_id in preferred_ids:
        ...
        if live_item is None:
            continue
        # Hide models that don't advertise tool-calling support — sidekick-agent
...
def fetch_ai_gateway_models(
    ...
    *,
    ...
    from runtime._compat.shim_constants import AI_GATEWAY_BASE_URL
...
def fetch_ai_gateway_pricing(
    ...
    *,
    ...
    """Fetch Vercel AI Gateway /v1/models and return sidekick-shaped pricing.

    Vercel uses ``input`` / ``output`` field names; sidekick's picker expects
...
def get_pricing_for_provider(provider: str, *, force_refresh: bool = False) -> dict[str, dict[str, str]]:
    """Return live pricing for providers that support it (openrouter, ai-gateway)."""
...
def list_available_providers() -> list[dict[str, str]]:
    ...
    source of truth shared with ``sidekick model``, ``/model``, etc.).
    """
    ...
    for pid in provider_order:
        ...
        has_creds = False
        try:
            from cli.auth import get_auth_status, has_usable_secret
            if pid == "custom":
...
def _resolve_static_model_alias(
    ...
    try:
        from cli.model_switch import MODEL_ALIASES
    except Exception:
        return None
...
def normalize_provider(provider: Optional[str]) -> str:
    """Normalize provider aliases to Sidekick' canonical provider ids.

    Note: ``"auto"`` passes through unchanged — use
    ``sidekick_cli.auth.resolve_provider()`` to resolve it to a concrete
...
def model_supports_fast_mode(model_id: Optional[str]) -> bool:
    """Return whether Sidekick should expose the /fast toggle for this model."""
...
def _resolve_copilot_catalog_api_key() -> str:
    ...
    Resolution order:
      ...
      2. ``read_credential_pool("copilot")`` — a token (typically a
         ...
         populated by ``sidekick auth add copilot`` and by ``_seed_from_env``
         when the env var is set in ``~/.sidekick/.env``.
    ...
    try:
        from cli.auth import read_credential_pool
        from cli.copilot_auth import (
...
def provider_model_ids(provider: Optional[str], *, force_refresh: bool = False) -> list[str]:
    ...
    Tries live API endpoints for providers that support them (Codex),
    ...
    if normalized == "openai-codex":
        ...
        # a Sidekick release). Falls back to the hardcoded catalog if no token
        # or the endpoint is unreachable.
        access_token = None
        try:
            from cli.auth import resolve_codex_runtime_credentials
...
def _fetch_anthropic_models(timeout: float = 5.0) -> Optional[list[str]]:
    ...
    """
    try:
        from runtime.anthropic_adapter import resolve_anthropic_token, _is_oauth_token
    except ImportError:
        return None
    ...
    if is_oauth:
        ...
        from runtime.anthropic_adapter import _COMMON_BETAS, _OAUTH_ONLY_BETAS, _CONTEXT_1M_BETA
...
def copilot_default_headers() -> dict[str, str]:
    ...
    """
    try:
        from cli.copilot_auth import copilot_request_headers
        return copilot_request_headers(is_agent_turn=True)
    except ImportError:
        return {
            "Editor-Version": COPILOT_EDITOR_VERSION,
            "User-Agent": "SidekickAgent/1.0",
...
def _lmstudio_request_headers(api_key: Optional[str] = None) -> dict:
    ...
    headers = {"User-Agent": _SIDEKICK_USER_AGENT}
...
_COPILOT_MODEL_ALIASES = {
    ...
    # Dash-notation fallbacks: Sidekick' default Claude IDs elsewhere use
    ...
    *,
    ...
    if normalized == "custom" or normalized.startswith("custom:"):
        ...
        message = (
            ...
            f"Sidekick will still save `{requested}`, but the endpoint should expose `/models` for verification."
        )
        if api_mode == "anthropic_messages":
            message += (
        ...
        if catalog_models:
            ...
            return {
                ...
                "message": (
                    ...
                    f"{suggestion_text}"
                    "\n  MiniMax does not expose a /models endpoint, so Sidekick cannot verify the model name."
    ...
    if normalized == "bedrock":
        try:
            from runtime.bedrock_adapter import discover_bedrock_models, resolve_bedrock_region
