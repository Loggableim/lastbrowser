...
def test_workspaces_endpoint_merges_space_engine_entries(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        web_server,
        "load_workspaces",
        lambda: [{"path": r"C:\Users\<user>\workspace", "name": "Home"}],
    )
    monkeypatch.setattr(web_server, "get_last_workspace", lambda: r"C:\Users\<user>\workspace")
    monkeypatch.setattr(
    ...
    assert response.status_code == 200
    payload = response.json()
    assert payload["last"] == r"C:\Users\<user>\workspace"
