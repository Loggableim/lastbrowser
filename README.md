...
Hermes WebUI ships with a **native Windows desktop app** (Electron-based), inspired by productivity browsers like [Sidekick](https://sidekickbrowser.com).

### Features
- **Sidekick-style sidebar** — one-click panel switching (Chat, Tasks, Agents, Skills, Memory, Workspaces, Profiles, Settings)
- **Keyboard shortcuts** — `⌘1` through `⌘8` for instant panel navigation
- **System tray** — minimize to tray, background operation, quick context menu
- **Python server lifecycle** — auto-starts/stops the WebUI Python backend
- **Auto-update** — via GitHub Releases (electron-updater)
- **Custom titlebar** — with native window controls overlay
...
cd native
npm install
npm run dev       # Dev mode (ELECTRON_DEV=1)
```
...
```bash
cd native
npm run build     # → native/dist/Hermes-Setup-*.exe
```

### CI/CD

Tag pushes (`v*`) trigger automatic builds via `.github/workflows/build-desktop.yml`:
1. Generate icons (Python + Pillow)
2. Install dependencies
3. Build NSIS installer with electron-builder
4. Publish to GitHub Releases

See [native/README.md](native/README.md) for full documentation.
