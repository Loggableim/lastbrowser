...
from runtime.account_usage import fetch_account_usage
...
def _account_usage_subprocess_env(home: Path, provider: str, api_key: str | None) -> dict[str, str]:
    ...
    try:
        from web.api.config import _AGENT_DIR
    except Exception:
        _AGENT_DIR = None
...
def _agent_fetch_account_usage_for_home(provider: str, home: Path, *, api_key: str | None = None) -> Any:
    try:
        from web.api.config import PYTHON_EXE
    except Exception:
...
def get_providers() -> dict[str, Any]:
        ...
        if pid == "nous":
            try:
                ...
                if live_ids:
                    ...
                    from web.api.config import _format_nous_label, _build_nous_featured_set
    ...
    active_provider = resolve_active_provider_context(cfg).get("provider")
...
def _clean_provider_key_from_config(provider_id: str) -> None:
    ...
    """
    from web.api.config import _cfg_lock
