...
def handle_get(handler, parsed) -> bool:
    ...
    if parsed.path.startswith("/api/workers/"):
        from api.remote_workers import handle_workers_get
    ...
    if parsed.path == "/api/terminal/stream":
        from api.streaming import _handle_terminal_stream
    ...
    if parsed.path == "/api/errors":
        from api.error_logger import get_errors
        qs = parse_qs(parsed.query)
    ...
    if parsed.path == "/api/errors/stats":
        from api.error_logger import get_error_stats
        return j(handler, {"success": True, "stats": get_error_stats()})

    if parsed.path == "/api/errors/db-path":
        from api.error_logger import get_db_path
        return j(handler, {"success": True, "path": get_db_path()})

    # ── Evey Tools API (GET) ──
    if parsed.path.startswith("/api/evey/"):
        from api.evey_tools import handle_evey_get
        return handle_evey_get(handler, parsed)

    # ── Discord Bot API (GET) ──
    if parsed.path.startswith("/api/discord/"):
        from api.discord_bot import handle_get
        return handle_get(handler, parsed)

    # ── Gmail API (GET) ──
    if parsed.path.startswith("/api/gmail/"):
        from api.gmail_tools import handle_gmail_get
        return handle_gmail_get(handler, parsed)

    # ── Freelancer API (GET) ──
    if parsed.path.startswith("/api/freelancer/"):
        print(f"[freelancer] GET matched: {parsed.path}", flush=True)
        from api.freelancer import (
            get_status,
            screenshot as fl_screenshot,
            form_fields,
        )
        subpath = parsed.path[len("/api/freelancer/"):]
        print(f"[freelancer] subpath: {subpath}", flush=True)
        if subpath == "status":
            result = get_status()
            print(f"[freelancer] status: {result}", flush=True)
            return j(handler, result)
        if subpath == "screenshot":
            try:
                img = fl_screenshot()
                return t(handler, img, content_type="image/png")
            except Exception as e:
                return j(handler, {"error": str(e)[:100]}, status=500)
        if subpath == "form-fields":
            try:
                return j(handler, form_fields())
            except Exception as e:
                return j(handler, {"error": str(e)[:100], "fields": [], "count": 0})
        return j(handler, {"error": "unknown endpoint"}, status=404)

    # ── Agents API (GET) ──
    if parsed.path == "/api/agents/splash/status":
        from api.agents import is_splash_completed
...
def handle_post(handler, parsed) -> bool:
    ...
    if parsed.path.startswith("/api/workers/"):
        from api.remote_workers import handle_workers_post
    ...
    if parsed.path == "/api/errors/clear":
        from api.error_logger import clear_errors
    ...
    if parsed.path.startswith("/api/discord/"):
        from api.discord_bot import handle_post
        return handle_post(handler, parsed, body)

    # ── Gmail API (POST) ──
    if parsed.path.startswith("/api/gmail/"):
        from api.gmail_tools import handle_gmail_post
    ...
    if parsed.path.startswith("/api/freelancer/"):
        ...
        try:
            if subpath == "start":
                return j(handler, start())
            if subpath == "stop":
                stop()
                return j(handler, {"status": "stopped"})
            if subpath == "navigate":
                url = body.get("url", "https://www.freelancer.com/login")
                return j(handler, navigate(url))
            if subpath == "click":
                return j(handler, click(body.get("x", 0), body.get("y", 0)))
            if subpath == "fill":
                return j(handler, fill(body.get("selector", ""), body.get("text", "")))
            if subpath == "evaluate":
                return j(handler, evaluate(body.get("js", "")))
            if subpath == "save-session":
                save_session()
                return j(handler, {"status": "saved"})
        except Exception as e:
            return j(handler, {"error": str(e)[:200]}, status=500)
        return j(handler, {"error": "unknown endpoint"}, status=404)
...
def _handle_agents_get(handler, parsed):
    ...
    if path == "/api/agents/workspaces":
        from api.agent_workspace import ensure_agent_workspace
        from api.agents import list_activated_agents
        agents_list = list_activated_agents()
        results = {}
        for a in agents_list:
    ...
    if path.startswith("/api/agents/workspace/stream/"):
        ...
        from api.agent_workspace import stream_events
        handler.send_response(200)
    ...
    if len(parts) == 5 and parts[4] == "workspace":
        from api.agent_workspace import create_workspace_session
        slug = parts[3]
        agent = get_agent(slug)
        if agent is None:
            return bad(handler, f"Agent '{slug}' not found", status=404)
...
def _handle_agents_post(handler, parsed, body):
    ...
    from api.helpers import require, j, bad
    ...
    if path == "/api/agents/splash/question":
        from api.agents import agent_creator_step
    ...
    if len(parts) == 6 and parts[4] == "workspace" and parts[5] == "process" and parts[2] == "agents":
        slug = parts[3]
        from api.agent_workspace import process_agent_request
        message = body.get("message", "")
    ...
    # POST /api/agents/<slug>/workspace/hermes — Hermes Agent Subprozess starten
    parts = path.split("/")
    if len(parts) == 6 and parts[4] == "workspace" and parts[5] == "hermes" and parts[2] == "agents":
        slug = parts[3]
        from api.agent_workspace import spawn_hermes_agent
        task = body.get("task", "")
        session_id = body.get("session_id")
        agent = get_agent(slug)
        workdir = agent.get("workdir", "") if agent else ""
        result = spawn_hermes_agent(slug, workdir, task, session_id)
        return j(handler, result)
    ...
    if path.startswith("/api/agents/workspace/") and path.endswith("/command"):
        ...
        from api.agent_workspace import send_command_to_agent
    ...
    if path.startswith("/api/agents/workspace/") and path.endswith("/stop"):
        session_id = path.split("/")[4]
        from api.agent_workspace import destroy_workspace_session
    ...
    if len(parts) == 5 and parts[4] == "chat":
        ...
        from api.agents import chat_with_agent
...
def _handle_agents_patch(handler, parsed, body):
    ...
    from api.agents import update_agent
    from api.helpers import j, bad
...
def _handle_hybrid_search(handler, body):
    ...
    # 1. Local search: search the MEMORY.md notes
    try:
        from api.profiles import get_active_hermes_home
