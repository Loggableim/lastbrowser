...
# Each HTTP request thread can set its own kanban home so that ``_conn()``
# opens the workspace-specific ``kanban.db`` instead of the global one.
_KANBAN_LOCAL = threading.local()
...
def clear_workspace_kanban() -> None:
    """Remove the per-thread override."""
    try:
        del _KANBAN_LOCAL.home
    except AttributeError:
        pass


def _get_ws_kanban_home() -> str | None:
    """Return the workspace kanban home path for this thread, or ``None``."""
    return getattr(_KANBAN_LOCAL, 'home', None)
