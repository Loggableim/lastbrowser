...
const _SESSION_LOAD_TIMEOUT_MS = 15000;
...
async function _sessionApi(path, timeoutMs, externalSignal) {
  ...
  try {
    ...
    clearTimeout(timer);
    if (externalSignal) {
      try { externalSignal.removeEventListener('abort', abortFromExternal); } catch (_) {}
    }
  }
}
  ...
  _liveStreamRehydrateTimer = setTimeout(async () => {
    ...
    try {
      ...
      if (S.session && S.session.session_id === sid) {
        ...
        if (session.pending_user_message) S.session.pending_user_message = session.pending_user_message;
        if (Array.isArray(session.pending_attachments)) S.session.pending_attachments = session.pending_attachments;
      }
      if (typeof attachLiveStream === 'function') {
        attachLiveStream(sid, streamId, session.pending_attachments || [], {reconnecting: true});
      }
    } catch (_) {}
  }, 1200);
}
...
async function newSession(flash, options={}){
  ...
  const spaceDefaultPath = window._activeSpaceConfig?.project_dir || null;
  const inheritWs = switchWs || spaceDefaultPath || (S.session ? S.session.workspace : null) || (S._profileDefaultWorkspace || null);
  ...
  const selectedDefaultModel = spaceDefaultModel || window._defaultModel || (modelSel && modelSel.value) || '';
  let defaultApplied=false;
  const effectiveDefault = spaceDefaultModel || window._defaultModel;
  if(effectiveDefault && modelSel && typeof _applyModelToDropdown==='function'){
    defaultApplied = !!_applyModelToDropdown(effectiveDefault, modelSel, spaceDefaultModel ? (window._activeSpaceConfig?.model?.provider || null) : (window._activeProvider || null));
    // Wenn Space-Default, auch den Provider mitgeben
    if(spaceDefaultModel && window._activeSpaceConfig?.model?.provider) {
      window._activeProvider = window._activeSpaceConfig.model.provider;
    }
  }
  const canQualify = !effectiveDefault || defaultApplied || (modelSel && modelSel.value === selectedDefaultModel);
  const newModelState = (canQualify && typeof _modelStateForSelect === 'function')
    ? _modelStateForSelect(modelSel,selectedDefaultModel)
    : { model: selectedDefaultModel, model_provider: spaceDefaultModel ? (window._activeSpaceConfig?.model?.provider || null) : null };
  const spaceQS = (typeof getActiveSpaceQuery === 'function') ? getActiveSpaceQuery() : '';
  const isSpaceScoped = !!spaceQS;
  const reqBody={
    model:newModelState.model,
    model_provider:newModelState.model_provider||null,
    profile:S.activeProfile||'default',
  };
  if(!isSpaceScoped && inheritWs) reqBody.workspace=inheritWs;
  ...
  // Wenn Space-Default aktiv ist, Dropdown darauf setzen
  if (spaceDefaultModel && S.session.model !== spaceDefaultModel && modelSel && typeof _applyModelToDropdown === 'function') {
    _applyModelToDropdown(spaceDefaultModel, modelSel, window._activeSpaceConfig?.model?.provider || null);
    if (typeof syncModelChip === 'function') syncModelChip();
  }
...
async function loadSession(sid, options){
  ...
  _activeSessionLoadAbortController = loadAbortController;
  ...
  if (currentSid !== sid) {
    ...
    if (_msgInner) {
      const showLoading = () => {
        if (_loadingSessionId !== sid) return;
        const inner = $('msgInner');
        if (inner) inner.innerHTML = '<div style="display:flex;align-items:center;justify-content:center;height:100%;color:var(--text-muted);font-size:14px;padding:40px;text-align:center;">Loading conversation...</div>';
      };
  ...
  const loadedSessionSpace = String(
    (data && data.session && (data.session.workspace_slug || data.session.space_slug || data.session.space)) || ''
  ).trim().toLowerCase();
  ...
  if (loadedSessionSpace && typeof _activeSpace !== 'undefined' && loadedSessionSpace !== _activeSpace) {
    ...
    // Refresh the token so the post-load stale checks do not abort the
    // session render just because we intentionally switched spaces.
    spaceLoadKey = (typeof _activeSpaceLoadKey === 'function') ? _activeSpaceLoadKey() : spaceLoadKey;
  }
  ...
  if(INFLIGHT[sid]){
    ...
    // Workspace tree refreshes can be slow; active chat switching must stay responsive.
    void loadDir('.');
    clearLiveToolCards();
    ...
    if(activeStreamId){
      ...
      setStatus('');
      setComposerStatus('');
      if(typeof _beginSessionFastRender==='function') _beginSessionFastRender();
      syncTopbar();renderMessages();appendThinking();
      // Workspace tree refreshes can be slow; active chat switching must stay responsive.
      void loadDir('.');
      updateQueueBadge(sid);
      ...
      // Kick off loadDir without blocking the session switch. Slow or invalid
      // workspace paths must not keep _loadingSessionId alive and make chat
  ...
  // Final live-stream safety net: some reloads restore the session snapshot
  // with active_stream_id but leave S.activeStreamId unset, which makes the UI
  // show a spinner without live actions. If that happens, re-attach once here.
  if (S.session && S.session.session_id === sid && S.session.active_stream_id && !S.activeStreamId) {
    S.activeStreamId = S.session.active_stream_id;
    S.busy = true;
    try { if (typeof updateSendBtn === 'function') updateSendBtn(); } catch (_) {}
    try { setStatus(''); setComposerStatus(''); } catch (_) {}
    try { syncTopbar(); renderMessages(); appendThinking(); } catch (_) {}
    try {
      if (typeof attachLiveStream === 'function') {
        attachLiveStream(sid, S.session.active_stream_id, S.session.pending_attachments || [], {reconnecting: true});
      }
    } catch (_) {}
  }
  ...
  if (_loadingSessionId === sid) _loadingSessionId = null;
  if (_activeSessionLoadAbortController === loadAbortController) {
    _activeSessionLoadAbortController = null;
  }
  // Defensive re-render: if a race or late guard left the conversation pane on
  // the loading placeholder even though S.session/S.messages are already ready,
  // force one final view sync on the next frame. This keeps fresh reloads and
  // fast space/session switches from getting stuck on "Loading conversation...".
  if (S.session && S.session.session_id === sid) {
    ...
    _scheduleConversationPaneRecovery(sid);
    if (!S.activeStreamId) {
      _scheduleLiveStreamRehydrate(sid);
    }
  }
...
let _sessionListInFlight = false;  // prevent request pileup
...
async function renderSessionList(){
  ...
  const activeWorkspace = (typeof getActiveSpaceQuery === 'function')
    ...
    : '';
  const requestKey = `${spaceLoadKey}|${activeWorkspace}|${_showAllProfiles ? 'all' : 'profile'}`;
  if(_sessionListInFlight && _sessionListInFlightPromise && _sessionListInFlightKey === requestKey) {
    return _sessionListInFlightPromise;
  }
  if (_sessionListAbortController) {
    try { _sessionListAbortController.abort(); } catch (_) {}
  }
  const listAbortController = new AbortController();
  _sessionListAbortController = listAbortController;
  const _gen = ++_renderSessionListGen;
  _sessionListInFlight = true;
  _sessionListInFlightKey = requestKey;
  const run = (async () => {
  try{
    _sessionListInFlight = true;
    ...
    if (_gen !== _renderSessionListGen) return;
    if (spaceLoadKey && typeof isActiveSpaceLoadKey === 'function' && !isActiveSpaceLoadKey(spaceLoadKey)) return;
    console.warn('renderSessionList',e);if(typeof _allSessions!=='undefined')_allSessions=[];
  }
  })();
  _sessionListInFlightPromise = run;
  try {
    return await run;
  } finally {
    if (_sessionListInFlightPromise === run) {
      _sessionListInFlight = false;
      _sessionListInFlightKey = '';
      _sessionListInFlightPromise = null;
      if (_sessionListAbortController === listAbortController) _sessionListAbortController = null;
    }
  }
}
...
function _spaceScopedApiPath(path) {
  ...
  params.set('workspace', activeSpace);
...
function renderSessionListFromCache(){
  ...
  if(_activeProject) _activeProject=null;
  const projectFiltered=profileFiltered;
  ...
  if(false&&(_allProjects.length>0||hasUnprojected)){
    const bar=document.createElement('div');
  ...
  if(archivedCount>0){
    ...
    toggle.textContent=_showArchived?t('hide_archived'):t('show_n_archived',archivedCount);
  ...
  if(_activeProject&&sessions.length===0){
    ...
    empty.textContent=_activeProject===NO_PROJECT_FILTER?t('no_unassigned_sessions'):t('no_sessions_in_project');
  ...
  if(curItems.length) groups.push({label:curLabel,items:curItems});
  // CLI sessions group (collapsed by default)
  ...
  for(const g of groups){
    ...
    const isGroupCollapsed=Boolean(_groupCollapsed[g.collapseKey||g.label]);
    if(isGroupCollapsed){body.style.display='none';caret.classList.add('collapsed');}
    hdr.onclick=()=>{
      const isCollapsed=body.style.display==='none';
      body.style.display=isCollapsed?'':'none';
      caret.classList.toggle('collapsed',!isCollapsed);
      const key=g.collapseKey||g.label;
      _groupCollapsed[key]=!isCollapsed;
      _saveCollapsed();
      renderSessionListFromCache();
    };
  ...
  function _renderOneSession(s, isPinnedGroup=false){
    ...
    el.onpointerup=(e)=>{
      ...
      _tapTimer=setTimeout(async()=>{
        ...
        await loadSession(s.session_id, {expectedSpace: (typeof _activeSpace !== 'undefined') ? _activeSpace : ''});renderSessionListFromCache();
...
async function deleteSession(sid){
  ...
  // Optimistic client-side removal: remove the session from the local
  // cache right away so the sidebar reflects the deletion immediately,
  // even if the subsequent renderSessionList() API-call (which re-deletes
  // _index.json and triggers a full directory scan) encounters a transient
  // failure and leaves the stale _allSessions in place (#sidebar-delete-stale).
  if(Array.isArray(_allSessions)){
