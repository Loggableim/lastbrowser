...
async function send(){
      ...
      if(text.startsWith('/')){
        ...
        if(_pc&&['steer','interrupt','queue','terminal','goal'].includes(_pc.name)){
          ...
          if(_bc){
            ...
            // Save steer/interrupt text to prompt history (it's the user's intent)
            if(typeof _addToPromptHistory==='function'&&text&&!text.startsWith('/'))_addToPromptHistory(text);
  ...
  // Save to prompt history for ArrowUp/Down recall
  if(typeof _addToPromptHistory==='function'&&text&&!text.startsWith('/'))_addToPromptHistory(text);
  $('msg').value='';autoResize();
  ...
  try{
    const startData=await api('/api/chat/start',{method:'POST',body:JSON.stringify({
      ...
      mode: window._composerMode||'code',
...
function attachLiveStream(activeSid, streamId, uploaded=[], options={}){
  ...
  function _wireSSE(source){
    ...
    source.addEventListener('tool',e=>{
      ...
      // Show live progress card (spinner + timer in #liveToolCards)
    ...
    source.addEventListener('tool_complete',e=>{
      ...
      persistInflightState();
      // Update live progress card in #liveToolCards
...
source.addEventListener('done',e=>{
    ...
    es.onerror = function() {
      ...
      if (_sseReconnectCount > _SSE_MAX_RECONNECT) {
        es.close();
        _sseReconnectCount = 0;
        console.warn('[sse] Max reconnections reached, falling back to polling');
      }
    };
    _wireSSE(es);
              return;
