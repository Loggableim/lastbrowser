...
// ── i18n: locale bundles and t() helper ──────────────────────────────────────
// To add a new language: create web/static/i18n/<code>.js exporting the keys
// and add an entry to LOCALE_LOADERS below.
// The language code must match a valid BCP 47 tag (used for speech recognition).
// Keys missing in a non-English locale fall back to English automatically.
//
// Only English ships inline (it is the fallback for every lookup). Every other
// locale is fetched on demand from web/static/i18n/<code>.js, which cuts the
// initial payload from ~668 KB to ~77 KB (backlog item 11).
...
const LOCALE_LOADERS = {
  ...
  'zh-Hant': () => import('./i18n/zh-Hant.js'),
...
function resolveLocale(lang) {
  ...
  const raw = lang.trim();
  if (!raw) return null;
  // A locale is known when it is either loaded (LOCALES) or loadable
  // (LOCALE_LOADERS) — non-English bundles are fetched on demand.
  const known = (code) => !!(LOCALES[code] || LOCALE_LOADERS[code]);
  if (known(raw)) return raw;

  const lower = raw.toLowerCase().replace(/_/g, '-');

  // Case-insensitive direct match first.
  const direct = Object.keys(LOCALE_LOADERS).concat(Object.keys(LOCALES))
  ...
  if (lower === 'zh' || lower.startsWith('zh-cn') || lower.startsWith('zh-sg') || lower.startsWith('zh-hans')) {
    return known('zh') ? 'zh' : null;
  }
  if (lower.startsWith('zh-tw') || lower.startsWith('zh-hk') || lower.startsWith('zh-mo') || lower.startsWith('zh-hant')) {
    return known('zh-Hant') ? 'zh-Hant' : null;
  }

  // Fallback to base language subtag (e.g. en-US -> en).
  const base = lower.split('-')[0];
  const baseMatch = Object.keys(LOCALE_LOADERS).concat(Object.keys(LOCALES))
    .find((k) => k.toLowerCase() === base);
...
/**
 * Fetch a locale bundle that is not shipped inline (everything except English).
 * Resolves true once the bundle is registered in LOCALES.
 * @param {string} code
 * @returns {Promise<boolean>}
 */
async function _ensureLocaleBundle(code) {
  if (!code || LOCALES[code]) return true;
  const loader = LOCALE_LOADERS[code];
  if (!loader) return false;
  try {
    await loader();
    const bundles = window.__SIDEKICK_LOCALE_BUNDLES__;
    if (bundles && bundles[code]) {
      LOCALES[code] = bundles[code];
      return true;
    }
  } catch (_) {
    // Network/parse failure: fall back to English rather than breaking the UI.
  }
  return false;
}

/**
 * Switch locale by language code, fetching the bundle when needed.
 * Persists to localStorage and updates the <html lang> attribute.
 * @param {string} lang
 * @returns {Promise<void>}
 */
async function setLocaleAsync(lang) {
  const resolved = resolveLocale(lang) || 'en';
  await _ensureLocaleBundle(resolved);
  setLocale(resolved);
}
 ...
 */
function loadLocale() {
  const resolved = resolvePreferredLocale(null, localStorage.getItem('sidekick-lang'));
  // English is inline; any other locale needs its bundle fetched first. The
  // synchronous pass applies English immediately so the UI is never blank,
  // then the real locale is applied once the bundle arrives.
  setLocale(resolved);
  if (resolved !== 'en') {
    _ensureLocaleBundle(resolved).then((ok) => {
      if (ok) {
        setLocale(resolved);
        applyLocaleToDOM();
      }
...
function applyLocaleToDOM() {
  ...
  // Also refresh elements tagged with data-i18n-key (dynamically created via t())
