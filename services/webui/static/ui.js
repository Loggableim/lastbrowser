...
async function fetchJson(path,opts={}){
  ...
  if(!res.ok){
    const errorValue = data && typeof data === 'object' ? data.error : null;
    const message = typeof errorValue === 'string'
      ? errorValue
      : (errorValue && typeof errorValue === 'object'
        ? (errorValue.message || errorValue.code || data.message || ('HTTP '+res.status))
...
window.addEventListener('beforeunload', () => _abortController.abort(), { once: true });
...
function _getModelProviderName(modelId){
  ...
  if(modelId.startsWith('@') && modelId.includes(':')){
    const provider=modelId.slice(1,modelId.lastIndexOf(':'));
    return _formatProviderName(provider);
  }
  // Extract from provider/model format (e.g. "deepseek/deepseek-v4-flash")
  const slashIdx=modelId.indexOf('/');
  if(slashIdx>0){
    const provider=modelId.slice(0,slashIdx);
    return _formatProviderName(provider);
  }
  // Fallback: try to get provider from the selected option's optgroup
  const opt=_selectedModelOption();
  if(opt){
    const providerId=_getOptionProviderId(opt);
    if(providerId) return _formatProviderName(providerId);
    const group=opt.parentElement;
    if(group&&group.tagName==='OPTGROUP'&&group.label){
      return _formatProviderName(group.label.replace(/\(\d+\)$/,'').trim());
    }
  }
  return '';
}
function _formatProviderName(raw){
  const name=String(raw||'').trim().toLowerCase();
  const SPECIAL={'openai':'OpenAI','anthropic':'Anthropic','deepseek':'DeepSeek','google':'Google','meta-llama':'Meta (Llama)','ollama':'Ollama','openrouter':'OpenRouter','github':'GitHub Models','azure':'Azure','aws':'AWS Bedrock','gcp':'GCP Vertex','together':'Together AI','fireworks':'Fireworks','groq':'Groq','mistral':'Mistral AI','cohere':'Cohere','perplexity':'Perplexity','xai':'xAI','replicate':'Replicate','huggingface':'HuggingFace','custom':'Custom'};
  if(SPECIAL[name]) return SPECIAL[name];
  return name.charAt(0).toUpperCase()+name.slice(1).replace(/[-_]/g,' ');
}
// Classify a model into a speed/cost tier for the colored dot indicator.
function _classifyModelSpeed(modelId){
  if(!modelId) return 'unknown';
  // Inline free-model check (model ID ends with :free)
  if(modelId.endsWith(':free')) return 'free';
  const id=modelId.toLowerCase();
  // Expensive/premium models (red)
  const slowPatterns=['o3','o4-pro','gpt-4o','claude-sonnet-4.6','claude-opus','gemini-ultra'];
  for(const p of slowPatterns){
    if(id.includes(p)) return 'slow';
  }
  // Balanced/mid-range models (yellow)
  const balancedPatterns=['deepseek-v4-pro','gemini-2.5-pro','gemini-3.1-pro','sonnet-4-5','claude-sonnet-4-5','gpt-5.4-mini','o4-mini','llama-4-beast'];
  for(const p of balancedPatterns){
    if(id.includes(p)) return 'balanced';
  }
  // Fast/efficient models (green)
  const fastPatterns=['flash','mini','haiku','scout','lite','deepseek-v4-flash','deepseek-chat','v3-0324','llama-4-scout'];
  for(const p of fastPatterns){
    if(id.includes(p)) return 'fast';
  }
  // Default to balanced for anything we haven't classified
  return 'balanced';
}
    ...
    if(configuredModels.length){
      ...
      for(const m of configuredModels){
        ...
        pinBtn1.onclick = async (e) => {
          ...
          if (!slug) return;
          const currentDefault = window._activeSpaceConfig?.model?.default || '';
          const newDefault = (currentDefault === m.value) ? '' : m.value;
          const newProvider = window._activeSpaceConfig?.model?.provider || '';
          await api('/api/space/config', {
            method: 'POST',
            body: JSON.stringify({ slug, model: { default: newDefault, provider: newProvider } })
          });
...
function _syncToolCallGroupSummaryStable(group){
  if(!group) return;
  ...
  if(label){
    const isCollapsed=group.classList.contains('tool-call-group-collapsed');
    const action=isCollapsed?'Show details':'Hide details';
    if(toolCount) label.textContent=`${action} (${toolCount} tool call${toolCount===1?'':'s'})`;
    else label.textContent=action;
  }
  if(durationEl){
...
_syncToolCallGroupSummary=_syncToolCallGroupSummaryStable;
...
function updateComposerModeChips(){
  ...
  if(!wrap) return;
  const mode=window._busyInputMode||'queue';
  const chips=wrap.querySelectorAll('.composer-mode-chip');
  for(const chip of chips){
    const cm=chip.getAttribute('data-mode');
    chip.classList.toggle('active',cm==='queue'&&mode==='queue'||cm==='steer'&&mode==='steer');
  }
}

function setComposerBusyMode(mode){
  if(!mode||(mode!=='queue'&&mode!=='steer')) mode='queue';
  window._busyInputMode=mode;
  updateComposerModeChips();
  updateSendBtn();
  // Sync the settings dropdown if it exists
  const sel=$('settingsBusyInputMode');
  if(sel) sel.value=mode;
  // Persist to server settings (fire-and-forget)
  try{
...
function setChatMode(mode){
  ...
  for(const btn of btns){btn.classList.toggle('active',btn.dataset.mode===mode);}
  if(mode==='code'){
    if(typeof openSplitTerminal==='function') openSplitTerminal();
    // Suppress empty state animations
    const es=$('emptyState');
    if(es){es.style.animation='none';}
  }else{
    if(typeof closeTerminalPane==='function') closeTerminalPane();
  }
  try{api('/api/settings',{method:'POST',body:JSON.stringify({chat_mode:mode})});}catch(_){}
}
...
async function setComposerMode(mode){
  ...
  try{api('/api/settings',{method:'POST',body:JSON.stringify({composer_mode:mode})});}catch(_){}
...
function updateContextGauge(percent, tokens, cost){
  ...
  const usage=$('ctxTooltipUsage');
  const tk=$('ctxTooltipTokens');
  const ct=$('ctxTooltipCost');
  if(usage) usage.textContent='Context: '+Math.round(pct)+'%';
  if(tk) tk.textContent='Tokens: '+((tokens||0).toLocaleString());
  if(ct&&cost){ct.textContent='~$'+cost.toFixed(4);ct.style.display='';}
  const wrap=$('ctxIndicatorWrap');
  if(wrap) wrap.style.display='';
}
  ...
  _ctxPollTimer=setInterval(async function(){
    ...
    try{
      ...
      if(d&&d.session){
        ...
        const tokens=d.session.total_tokens||0;
        const maxCtx=d.session.max_context||128000;
        const pct=maxCtx>0?Math.min(95,(tokens/maxCtx)*100):Math.min(95,msgCount*5);
        const cost=d.session.estimated_cost||0;
        updateContextGauge(pct,tokens,cost);
      }
    }catch(_){}
  },5000);
}
function stopCtxPolling(){if(_ctxPollTimer){clearInterval(_ctxPollTimer);_ctxPollTimer=null;}}

function handleFileDrop(files){
  if(!S||!files||!files.length) return;
  for(const file of files){
    S.pendingFiles.push({file, name: file.name, size: file.size, type: file.type});
  }
 ...
 * composerBox (the textarea + attached-tray area).
 *
 * Enforces a max of 10 files per drop from the OS.
 */
function initDragDrop(){
  ...
  document.addEventListener('dragenter',function(e){
    ...
    if(!types) return;
    const hasFiles=Array.from(types).some(function(t){return t==='Files'||t==='application/ws-path';});
    if(!hasFiles) return;
    _dragCount++;
  ...
  document.addEventListener('dragleave',function(e){
    _dragCount--;
    if(_dragCount<=0){_dragCount=0;wrap.classList.remove('drag-over');}
  });
  ...
  document.addEventListener('drop',function(e){
    ...
    if(wsPath){
      const msgEl=$('msg');
      if(msgEl){
        var start=msgEl.selectionStart,end=msgEl.selectionEnd,val=msgEl.value;
        var prefix=start>0&&!val[start-1].match(/\s/)?' ':'';
        var insert=prefix+'@'+wsPath+' ';
    ...
    if(files.length>MAX&&typeof showToast==='function'){
      showToast('Max '+MAX+' files per drop. First '+MAX+' of '+files.length+' files attached.',4000,'warning');
    }
...
async function toggleBgMode(){
  const inp=$('msg');
  const text=inp?inp.value.trim():'';
  if(!text){showToast('Type a message first to send as background task');return;}
  // If /bg command exists, use it
  if(typeof cmdBackground==='function'){
    inp.value='';autoResize();
    await cmdBackground(text);
  } else if(typeof executeCommand==='function'){
    inp.value='';autoResize();
    executeCommand('/bg '+text);
  } else {
    showToast('Background command not available');
  }
}
...
function _editQueuedMessageInComposer(sid, entry) {
  ...
  const q = _getSessionQueue(sid, false);
  if (!q.length) return;
  const entryTs = entry && entry._queued_at;
  const idx = entryTs != null ? q.findIndex(e => e && e._queued_at === entryTs) : -1;
  if (idx === -1) return;

  // Pull entry out of queue
  ...
  const msgEl = $('msg');
  if (msgEl) {
    msgEl.value = removed.text || removed.message || removed.content || '';
  ...
  // Restore files to pending tray
  if (removed.files && Array.isArray(removed.files) && removed.files.length) {
    S.pendingFiles = [...removed.files];
    if (typeof renderTray === 'function') renderTray();
  }
  ...
  if (typeof showToast === 'function') {
    showToast('Message moved to composer for editing', 1500, 'info');
  }
}
...
function _renderQueueChips(sid){
  ...
  q.forEach((entry,i)=>{
    ...
    msgSpan.onclick=(e)=>{
      e.stopPropagation();
      _editQueuedMessageInComposer(sid, entry);
    };
...
function _thinkingCardHtml(text){
  ...
  return `<div class="reasoning-accordion"><div class="reasoning-accordion-header" onclick="toggleReasoningAccordion(this)"><span>\u{1F9E0}</span><span class="reasoning-accordion-label">${t('reasoning_thought')}</span><span class="chevron">${li('chevron-right',12)}</span></div><div class="reasoning-accordion-body"><pre>${esc(clean)}</pre></div></div>`;
}
...
function toggleReasoningAccordion(btn){
...
// Persist expanded code-block state across renderMessages() DOM rebuilds
let _expandedCodeSigs = new Set();
function _codeSig(pre) {
  const txt = pre.textContent || '';
  const lines = txt.split('\n');
  const first = (lines[0] || '').trim().slice(0, 50);
  const last = (lines[lines.length - 1] || '').trim().slice(0, 30);
  return first + '\xAC' + last + '\xAC' + lines.length;
}
function _saveExpandedCodeBlocks() {
  _expandedCodeSigs.clear();
  document.querySelectorAll('#msgInner .code-collapse-expanded pre').forEach(pre => {
    _expandedCodeSigs.add(_codeSig(pre));
  });
}

// Persist expanded long-message state across renderMessages() DOM rebuilds.
// Live polling and stream updates recreate message DOM; without this, "Show more"
// snaps shut again while the user scrolls through a long answer.
let _expandedMessageCollapseKeys = new Set();
function _messageCollapseKey(seg) {
  const sid = S && S.session ? S.session.session_id || '' : '';
  const raw = seg && seg.dataset ? String(seg.dataset.rawText || '') : '';
  if (raw) return sid + ':raw:' + raw.length + ':' + raw.slice(0, 120) + ':' + raw.slice(-80);
  const idx = seg && seg.dataset ? seg.dataset.msgIdx || '' : '';
  return sid + ':idx:' + idx;
}
function _saveExpandedMessageCollapses() {
  document.querySelectorAll('#msgInner .assistant-segment .msg-body.msg-collapsible:not(.msg-collapsed)').forEach(body => {
    const seg = body.closest('.assistant-segment');
    if (seg) _expandedMessageCollapseKeys.add(_messageCollapseKey(seg));
  });
}

function _clipCliToolSnippet(text, maxLen=2000){
  const s=String(text||'');
...
const _LARGE_MESSAGE_PREVIEW_CHARS = 6000;
const _largeMessageRawByKey = new Map();
let _largeMessagePreviewSid = null;
window._sessionFastRenderMode = false;
window._sessionFastRenderToken = 0;
function _beginSessionFastRender() {
  window._sessionFastRenderMode = true;
  window._sessionFastRenderToken = (Number(window._sessionFastRenderToken || 0) + 1) | 0;
  return window._sessionFastRenderToken;
}
function _endSessionFastRender(token, sid) {
  setTimeout(() => {
    if (window._sessionFastRenderToken !== token) return;
    if (!S.session || S.session.session_id !== sid) return;
    window._sessionFastRenderMode = false;
    try { renderMessages({preserveScroll:true}); } catch (_) {}
  }, 900);
}
function _largeMessageKey(sid, rawIdx){
  return `${sid || ''}:${rawIdx}`;
}
function _largeMessagePreviewHtml(rawText, sid, rawIdx){
  const text = String(rawText || '');
  if (text.length <= _LARGE_MESSAGE_PREVIEW_CHARS) return null;
  const key = _largeMessageKey(sid, rawIdx);
  _largeMessageRawByKey.set(key, text);
  const preview = text.slice(0, _LARGE_MESSAGE_PREVIEW_CHARS).trimEnd();
  return (
    `<pre class="large-message-preview-text">${esc(preview)}</pre>` +
    `<div class="large-message-preview">` +
      `<div class="large-message-preview-note">Large response preview (${Math.round(text.length / 1024)} KB). Full content is kept and can be expanded on demand.</div>` +
      `<button type="button" class="btn small large-message-expand" onclick="_expandLargeMessage(this,'${escAttr(key)}')">Show full message</button>` +
    `</div>`
  );
}
...
function renderMessages(options){
  ...
  const sid=S.session?S.session.session_id:null;
  const fastRender=!!window._sessionFastRenderMode;
  if (sid !== _largeMessagePreviewSid) {
    _largeMessageRawByKey.clear();
    _largeMessagePreviewSid = sid;
  }
  ...
  for(let vi=0;vi<renderVisWithIdx.length;vi++){
    ...
    const largePreviewHtml = (!isUser && !m._live && !fastRender)
      ? _largeMessagePreviewHtml(_stripXmlToolCallsDisplay(String(displayContent)), sid, rawIdx)
      : null;
    let bodyHtml = isUser ? _renderUserFencedBlocks(displayContent) : (fastRender ? esc(_stripXmlToolCallsDisplay(String(displayContent))) : (largePreviewHtml || renderMd(_stripXmlToolCallsDisplay(String(displayContent)))));
    ...
    const stopBtn  = (!isUser && isLastAssistant && S.session && S.session.active_stream_id && typeof cancelStream==='function')
      ...
      : '';
    const resultToggleHtml = '';
    const stoppedBadgeHtml = (!isUser && (m._stopped || m.stopped || m.cancelled))
      ? `<div class="msg-stopped-badge">${t('stream_stopped')||'Stopped'}</div>`
      : '';
    ...
    if(m._isPlan){
      ...
      if(hasOnlyPlanCard){
        ...
        continue; // skip the normal segment body rendering below
      }
    }
  ...
  if(!S.busy){
    ...
    if(isSimplifiedToolCalling()){
      ...
      for(const [key, cards] of Object.entries(byAssistant)){
        ...
        // Wrap tool cards in a collapsible activity group
        const group=ensureActivityGroup(anchorParent,{collapsed:true,anchor:anchorRow,activityKey:`assistant:${aIdx}`});
        const sourceMsg=S.messages[aIdx]||{};
        if(sourceMsg._turnDuration!==undefined) group.setAttribute('data-turn-duration', String(sourceMsg._turnDuration));
        const body=group&&group.querySelector('.tool-call-group-body');
        if(!body) continue;
        
        // Add thinking cards if present (inside the collapsible group body)
        const thinkingText=assistantThinking.get(aIdx);
        if(thinkingText) body.appendChild(_thinkingActivityNode(thinkingText));
  ...
  // Re-append inline approval card after render if still pending
  if (typeof _inlineApprovalState !== 'undefined' && _inlineApprovalState && _inlineApprovalState.element && _inlineApprovalState.status === 'pending') {
    const existing = inner.querySelector('.inline-approval-card');
    if (!existing) {
...
function buildToolCard(tc, rawIdx){
  const row=document.createElement('div');
  ...
  const _fixCtx = _isTermErr && typeof _terminalErrorContext === 'function'
    ? _terminalErrorContext(tc) : '';
  const _fixCtxEsc = _fixCtx ? esc(JSON.stringify(_fixCtx)) : '';
...
/**
 * Handle "Fix this?" button click on a failed terminal tool card.
 * Pre-fills the composer with `/fix \n<error context>` and focuses input.
 */
function _fixThisTool(btn) {
  if (!btn || btn.disabled) return;
  // Read error context from data attribute (JSON-escaped string)
  let ctx = '';
  try {
    const raw = btn.getAttribute('data-fix-ctx') || '';
    if (raw) ctx = JSON.parse(raw);
  } catch(_) { ctx = 'Terminal command failed (error context unavailable).'; }
  // Pre-fill the composer with /fix command + error context
  const msgEl = $('msg');
  if (msgEl) {
    const fixText = '/fix\n\n```\n' + (ctx || 'Terminal command failed') + '\n```';
    msgEl.value = fixText;
    if (typeof autoResize === 'function') autoResize();
    msgEl.focus();
...
function appendLiveToolCard(tc){
  ...
  if(!isSimplifiedToolCalling()){
    ...
    const anchor=children.filter(el=>el.matches('[data-live-assistant="1"],.tool-card-row,.reasoning-accordion-row')).pop();
...
function renderToolLiveCard(toolId, toolName, status, startTime){
  ...
  if(!c) return;
  if(_toolLiveCards[toolId]) return; // deduplicate

  c.style.display='block';
  const card=document.createElement('div');
  card.className='tool-live-card';
  card.dataset.toolId=toolId;
  card.innerHTML='<div class="tool-live-header">'
    +'<span class="tool-live-icon spinner-icon"></span>'
    +'<span class="tool-live-name">'+esc(toolName)+'</span>'
    +'<span class="tool-live-time" id="toolTime_'+toolId+'">0.0s</span>'
    +'<span class="tool-live-status running">running\u2026</span>'
    +'</div>';
  c.appendChild(card);
  _toolLiveCards[toolId]={card,toolName};
  _startToolTimer(toolId,startTime||Date.now());
}
 ...
 */
function updateToolLiveCard(toolId,opts={}){
  const entry=_toolLiveCards[toolId];
  if(!entry) return;
  _stopToolTimer(toolId);
  const card=entry.card;
  if(!card||!card.isConnected){delete _toolLiveCards[toolId];return;}
  const st=opts.status||'done';
  if(st==='done'||st==='completed'){
    card.querySelector('.tool-live-icon').outerHTML='<span class="tool-live-icon done-icon">\u2713</span>';
    card.classList.add('tool-live-done');
    const se=card.querySelector('.tool-live-status');
    if(se){se.textContent='Done';se.className='tool-live-status done';}
  }else{
    card.querySelector('.tool-live-icon').outerHTML='<span class="tool-live-icon error-icon">\u2717</span>';
    card.classList.add('tool-live-error');
    const se=card.querySelector('.tool-live-status');
    if(se){se.textContent=opts.error||'Error';se.className='tool-live-status error';}
  }
  // fade out after a brief pause so user sees the state transition
  clearTimeout(entry._fadeTimer);
  entry._fadeTimer=setTimeout(function(){
    card.classList.add('tool-live-fadeout');
    setTimeout(function(){
      if(card.isConnected)card.remove();
      delete _toolLiveCards[toolId];
      _pruneLiveToolCards();
    },400);
  },800);
}

function _startToolTimer(toolId,startTime){
  if(_toolTimers[toolId]) return;
  var el=document.getElementById('toolTime_'+toolId);
  if(!el) return;
  _toolTimers[toolId]=setInterval(function(){
    var e=document.getElementById('toolTime_'+toolId);
    if(!e||!e.isConnected){_stopToolTimer(toolId);return;}
    e.textContent=((Date.now()-startTime)/1000).toFixed(1)+'s';
  },100);
}

function _stopToolTimer(toolId){
  if(_toolTimers[toolId]){clearInterval(_toolTimers[toolId]);delete _toolTimers[toolId];}
}

/** Hide #liveToolCards when empty. */
function _pruneLiveToolCards(){
  var c=$('liveToolCards');
  if(c&&!c.children.length)c.style.display='none';
}

/**
 * Clear ALL live tool cards (called on 'done' SSE event via clearLiveToolCards).
 */
function _clearAllLiveToolCards(){
  Object.keys(_toolTimers).forEach(_stopToolTimer);
  Object.keys(_toolLiveCards).forEach(function(id){
    if(_toolLiveCards[id]){clearTimeout(_toolLiveCards[id]._fadeTimer);delete _toolLiveCards[id];}
  });
  var c=$('liveToolCards');
...
function editMessage(btn) {
  ...
  bar.innerHTML = `<button class="msg-edit-send">${t('edit_save')}</button><button class="msg-edit-cancel">${t('edit_cancel')}</button>`;
  ta.after(bar);
...
async function submitEdit(msgIdx, newText) {
  ...
  // Try branching first (creates a new forked session)
  try {
    ...
    if(data && data.session_id) {
      if(typeof loadSession==='function') await loadSession(data.session_id);
      if(typeof renderSessionList==='function') await renderSessionList();
      showToast('Forked into new session');
      return;
    }
  } catch(_) {}
  // Fallback: truncate in-place and resend
  try {
      ...
      if(!wrapper) {
        ...
        toggleBtn.onclick = function(e) {
          ...
          if(expanded) {
            ...
            pre.style.maxHeight = '';
            _expandedCodeSigs.delete(_codeSig(pre));
          } else {
        ...
        if(_expandedCodeSigs.has(_codeSig(pre))) {
          wrapper.classList.add('code-collapse-expanded');
          toggleBtn.setAttribute('data-expanded', 'true');
          toggleBtn.textContent = t('show_less') || 'Show less';
          pre.style.maxHeight = pre.scrollHeight + 'px';
        }
...
const _codeLangExtMap={
  ...
  swift:'swift', php:'php', c:'c', cpp:'cpp', 'c++':'cpp',
  'c#':'cs', csharp:'cs', bash:'sh', sh:'sh', shell:'sh',
  zsh:'sh', fish:'sh', sql:'sql', html:'html', css:'css',
  scss:'scss', less:'less', yaml:'yml', yml:'yml', json:'json',
  xml:'xml', markdown:'md', md:'md', dockerfile:'Dockerfile',
  makefile:'Makefile', diff:'diff', patch:'patch',
};
...
function addCopyButtons(container){
  ...
  el.querySelectorAll('pre > code').forEach(codeEl=>{
    const pre=codeEl.parentElement;
    const header=pre.previousElementSibling;
    // Skip if toolbar already exists on this block
    if(pre.querySelector('.code-toolbar')) return;
    if(header&&header.classList.contains('pre-header')&&header.querySelector('.code-toolbar')) return;

    // Detect language: prefer pre-header text, fallback to code class
    let lang='';
    if(header&&header.classList.contains('pre-header')){
      lang=header.textContent.trim();
    }else{
    ...
    if(header&&header.classList.contains('pre-header')){
      ...
      toolbar.style.cssText='position:absolute;top:6px;right:6px;';
      pre.appendChild(toolbar);
    }
  });
}

/** Create a toolbar button with icon + label span */
function _codeToolbarBtn(icon,label){
  const btn=document.createElement('button');
  btn.className='code-toolbar-btn';
  btn.innerHTML='<span class="toolbar-icon">'+esc(icon)+'</span><span class="toolbar-label">'+esc(label)+'</span>';
  return btn;
}

/** Save code block content to a file in the workspace */
async function _codeBlockSave(codeEl,lang){
  const code=codeEl.textContent;
  const ext=_codeLangExtMap[lang]||lang||'txt';
  const filename=window.prompt('Save code as:', 'code.'+ext);
  if(!filename) return;
...
/** Run code block content in the embedded terminal */
async function _codeBlockRun(codeEl,lang){
  const code=codeEl.textContent;
  // Open terminal panel (if available)
  if(typeof toggleComposerTerminal==='function'){
    try{
      await toggleComposerTerminal(true);
      await new Promise(r=>setTimeout(r,200));
    }catch(e){/* terminal may fail silently */}
  }
  const sid=(typeof TERMINAL_UI!=='undefined'&&TERMINAL_UI.sessionId)
    || (S.session&&S.session.session_id);
  if(!sid){showToast('No terminal session available');return;}
  try{
...
function _initDiffActions(){
  ...
  document.querySelectorAll('pre.diff-block:not([data-diff-actions-ready])').forEach((pre,idx)=>{
    pre.dataset.diffActionsReady='1';
    const patchText=(pre.innerText||pre.textContent||'').trim();
    const looksApplicable=/^diff --git /m.test(patchText)||(/^---\s+/m.test(patchText)&&/^\+\+\+\s+/m.test(patchText));
    let wrapper=pre.closest('.diff-wrapper');
    if(!wrapper){
      wrapper=document.createElement('div');
    ...
    if(wrapper.querySelector(':scope > .diff-actions')) return;
    const patchId=pre.dataset.patchId||`diff-${Date.now()}-${idx}`;
    pre.dataset.patchId=patchId;
    const actions=document.createElement('div');
    actions.className='diff-actions';
    actions.innerHTML=[
      looksApplicable?'<button type="button" class="diff-action-btn diff-accept">Accept patch</button>':'',
      looksApplicable?'<button type="button" class="diff-action-btn diff-reject">Reject</button>':'',
      '<button type="button" class="diff-action-btn diff-edit-btn">Copy diff</button>',
    ].join('');
    ...
    const accept=actions.querySelector('.diff-accept');
    if(accept){
      accept.addEventListener('click',async()=>{
        try{
          setBusy(true);
          const res=await api('/api/chat/apply-patch',{
            method:'POST',
            body:JSON.stringify({patchId,action:'accept',patch_content:patchText}),
          });
          if(res&&res.ok===false) throw new Error(res.error||'Patch apply failed');
          wrapper.dataset.accepted='1';
          notify('Patch applied','success');
        }catch(err){
          notify('Patch apply failed: '+(err&&err.message?err.message:err),'error');
          setBusy(false);
        }
      });
    }
    const reject=actions.querySelector('.diff-reject');
    if(reject){
      reject.addEventListener('click',async()=>{
        try{
          setBusy(true);
          await api('/api/chat/apply-patch',{
            method:'POST',
            body:JSON.stringify({patchId,action:'reject'}),
          });
          wrapper.dataset.rejected='1';
          notify('Patch rejected','info');
        }catch(err){
          notify('Reject failed: '+(err&&err.message?err.message:err),'error');
          setBusy(false);
        }
      });
    }
    const copy=actions.querySelector('.diff-edit-btn');
    if(copy){
...
let _thinkingTimers={label:null,tools:null};
function _clearThinkingTimers(){
  if(_thinkingTimers.label){clearTimeout(_thinkingTimers.label);_thinkingTimers.label=null;}
  if(_thinkingTimers.tools){clearTimeout(_thinkingTimers.tools);_thinkingTimers.tools=null;}
}
function _updateThinkingToolsSummary(containerEl){
...
function finalizeThinkingCard(){
  ...
  if(!isSimplifiedToolCalling()){
    ...
    if(_scrollPinned){
      const body=row&&row.querySelector('.reasoning-accordion-body');
...
function appendThinking(text=''){
  ...
  if(!S.session||(!S.activeStreamId&&!S.busy)) return;
  _clearThinkingTimers();
  ...
  if(!isSimplifiedToolCalling()){
    ...
    if(!String(text||'').trim()){
      ...
      _thinkingTimers.tools=setTimeout(()=>{
        const tools=row&&row.querySelector('.thinking-indicator-tools');
        if(tools) _updateThinkingToolsSummary(tools);
      },30000);
    }
    scrollIfPinned();
  ...
  scrollIfPinned();
  if(_scrollPinned){
    const thinkingBody=row&&row.querySelector('.reasoning-accordion-body');
...
function addStreamCursor(){
  ...
  const body=liveSegments[liveSegments.length-1].querySelector('.msg-body');
  if(!body) return;
...
function initSplitPane(){
  ...
  if(!handle) return;
  const layout=handle.closest('.codex-split-layout');
  if(!layout) return;
...
async function saveEditedMessage(idx){
  const ta=document.querySelector('.msg-edit-area');
  if(!ta) return;
  const newText=ta.value.trim();
  if(!newText){showToast('Message cannot be empty');return;}
  if(!S.session){showToast('No active session');return;}
  showToast('Branching session...');
  try{
    const data=await api('/api/session/branch',{method:'POST',body:JSON.stringify({session_id:S.session.session_id,message_idx:idx,new_text:newText})});
    if(data&&data.session_id){
      if(typeof loadSession==='function') await loadSession(data.session_id);
      if(typeof renderSessionList==='function') await renderSessionList();
      showToast('Forked into new session');
    } else if(data&&data.error){
      showToast('Branch failed: '+data.error);
    } else {
      showToast('Branch created');
    }
  }catch(e){
    showToast('Branch error: '+e.message);
  }
}
...
function _togglePlanCollapse(btn){
  const card=btn.closest('.plan-card');
  if(!card) return;
  const seg=card.closest('.assistant-segment');
  ...
  if(card.classList.contains('plan-card-collapsed')){
    ...
    btn.textContent='+';
    if(rawIdx!==null && !isNaN(rawIdx)) _PLAN_COLLAPSED.add(rawIdx);
  }
}

async function _acceptPlan(planId, rawIdx){
  if(!S.session) return;
  const sid=S.session.session_id;
  if(!sid) return;
...
function _cancelPlanRevision(rawIdx){
  const form=document.getElementById('planReviseForm_'+rawIdx);
  if(!form) return;
...
async function _submitPlanRevision(rawIdx){
  if(!S.session) return;
  const sid=S.session.session_id;
  if(!sid) return;
  const form=document.getElementById('planReviseForm_'+rawIdx);
  ...
  if(!feedback){
    if(typeof showToast==='function') showToast('Please enter revision feedback',2000);
    ta.focus();
    return;
  }
...
function _applyCollapsibleMessages(container) {
  ...
  if (!container) return;
  var segments = container.querySelectorAll('.assistant-segment:not([data-live-assistant])');
  for (var i = 0; i < segments.length; i++) {
    var seg = segments[i];
    var existingToggle = seg.querySelector('.msg-collapse-toggle');
    var body = seg.querySelector('.msg-body');
    if (!body) continue;
    if (existingToggle) {
      var existingKey = _messageCollapseKey(seg);
      var existingOpen = _expandedMessageCollapseKeys.has(existingKey);
      body.classList.toggle('msg-collapsed', !existingOpen);
      var nextText = existingOpen ? 'Show less' : 'Show more';
      var nextLabel = existingOpen ? 'Collapse message' : 'Expand message';
      if (existingToggle.textContent !== nextText) existingToggle.textContent = nextText;
      if (existingToggle.getAttribute('aria-label') !== nextLabel) existingToggle.setAttribute('aria-label', nextLabel);
      continue;
    }
    if (seg._cbCp) continue;
    var textLen = (body.textContent || '').trim().length;
    if (textLen < 80) continue;
    var wasCol = body.classList.contains('msg-collapsed');
    body.classList.remove('msg-collapsible', 'msg-collapsed');
    var h = body.scrollHeight;
    if (h <= 320) {
      if (wasCol) body.classList.add('msg-collapsible', 'msg-collapsed');
      continue;
    }
    seg._cbCp = true;
    var key = _messageCollapseKey(seg);
    var isOpen = _expandedMessageCollapseKeys.has(key);
    body.classList.add('msg-collapsible');
    if (!isOpen) body.classList.add('msg-collapsed');
    var tg = document.createElement('button');
    tg.className = 'msg-collapse-toggle';
    tg.textContent = isOpen ? 'Show less' : 'Show more';
    tg.setAttribute('aria-label', isOpen ? 'Collapse message' : 'Expand message');
    tg.addEventListener('click', (function(b, collapseKey) {
      return function(e) {
        if(e){
          e.preventDefault();
          e.stopPropagation();
        ...
        var c = b.classList.toggle('msg-collapsed');
        this.textContent = c ? 'Show more' : 'Show less';
        this.setAttribute('aria-label', c ? 'Expand message' : 'Collapse message');
        if (c) _expandedMessageCollapseKeys.delete(collapseKey);
        else {
          _expandedMessageCollapseKeys.add(collapseKey);
          _scrollPinned=false;
          _messageUserUnpinned=true;
        }
        if(scrollBox && !c){
          requestAnimationFrame(function(){
            scrollBox.scrollTop = oldTop;
          });
        }
      };
    })(body, key));
    body.parentNode.insertBefore(tg, body.nextSibling);
  }
  } finally {
    _applyCollapsibleMessages._running = false;
  }
}
...
function syncComposerModeButtons(mode){
  const normalized=mode==='plan' ? 'plan' : 'action';
  const btns=document.querySelectorAll('.mode-btn');
  for(const btn of btns){
...
function _bindComposerModeToggle(){
  const wrap=$('modeToggle');
  if(!wrap){ setTimeout(_bindComposerModeToggle,100); return; }
  if(wrap.dataset.composerModeBound==='1') return;
  wrap.dataset.composerModeBound='1';
