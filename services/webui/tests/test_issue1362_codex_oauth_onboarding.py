...
def test_start_imports_existing_codex_cli_tokens_before_device_request(monkeypatch, tmp_path):
    import api.oauth as oauth

    oauth._OAUTH_FLOWS.clear()
    active_home = tmp_path / "lastbrowser-runtime"
    active_home.mkdir()
    ...
    monkeypatch.setattr(Path, "home", lambda: tmp_path)
    monkeypatch.setattr(oauth, "_get_active_hermes_home", lambda: active_home)
    monkeypatch.setattr(oauth, "_request_codex_user_code", lambda: pytest.fail("device code endpoint should not be called"))
    ...
    store = json.loads((active_home / "auth.json").read_text(encoding="utf-8"))
    entry = store["credential_pool"]["openai-codex"][0]
    assert entry["access_token"] == "codex-access"
    assert entry["refresh_token"] == "codex-refresh"
    assert entry["source"] == "codex_cli_import"


def test_start_codex_oauth_530_error_is_actionable(monkeypatch, tmp_path):
    import api.oauth as oauth
    ...
    def _raise_530():
        raise urllib.error.HTTPError(
            ...
            BytesIO(b'{"error":{"code":530,"message":"A 1xxx error occured.","type":"cf_route_error"}}'),
        )

    monkeypatch.setattr(oauth, "_request_codex_user_code", _raise_530)

    with pytest.raises(RuntimeError) as exc:
        oauth.start_onboarding_oauth_flow({"provider": "openai-codex"})

    message = str(exc.value)
    assert "OpenAI Codex login is temporarily unavailable" in message
    assert "HTTP 530" in message
    assert "~/.codex/auth.json" in message
    assert "cf_route_error" in message
...
def test_codex_credentials_written_to_active_profile_auth_json(monkeypatch, tmp_path):
    ...
    if os.name != "nt":
        ...
        # Windows exposes only a read-only bit through os.chmod/stat here.
        assert auth_path.exists()
