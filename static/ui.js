...
function renderMessages(options){
  ...
  requestAnimationFrame(()=>{highlightCode();addCopyButtons();loadDiffInline();loadCsvInline();loadExcalidrawInline();loadPdfInline();loadHtmlInline();renderMermaidBlocks();renderKatexBlocks();_applyCollapsibleMessages(inner);});
  requestAnimationFrame(()=>{highlightCode();addCopyButtons();initTreeViews();loadPdfInline();loadHtmlInline();renderMermaidBlocks();renderKatexBlocks();});
...
function _applyCollapsibleMessages(container) {
  ...
  for (var i = 0; i < segments.length; i++) {
    ...
    if (seg.querySelector('.msg-collapse-toggle')) continue;
    ...
    seg._cbCp = true;
    body.classList.add('msg-collapsible', 'msg-collapsed');
    var tg = document.createElement('button');
    tg.className = 'msg-collapse-toggle';
    tg.textContent = 'Show more ▼';
    tg.setAttribute('aria-label', 'Expand message');
    tg.addEventListener('click', (function(b) {
      return function() {
...
(function(){
  function init(){
    ...
    var obs = new MutationObserver(function(){ _applyCollapsibleMessages(inner); });
